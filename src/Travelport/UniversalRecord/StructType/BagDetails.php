...
class BagDetails extends AbstractStructBase
{
     ...
     * @var \Travelport\UniversalRecord\StructType\AvailableDiscount[]
     */
     ...
     * @param \Travelport\UniversalRecord\StructType\AvailableDiscount[] $availableDiscount
    ...
    public static function validateBaggageRestrictionForArrayConstraintsFromSetBaggageRestriction(?array $values = []): string
    {
        ...
        foreach ($values as $bagDetailsBaggageRestrictionItem) {
            ...
            if (!$bagDetailsBaggageRestrictionItem instanceof \Travelport\UniversalRecord\StructType\BaggageRestriction) {
    ...
    /**
     * This method is responsible for validating the values passed to the setAvailableDiscount method
     ...
     * @return string A non-empty message if the values does not match the validation rules
     */
    public static function validateAvailableDiscountForArrayConstraintsFromSetAvailableDiscount(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $bagDetailsAvailableDiscountItem) {
            // validation for constraint: itemType
            if (!$bagDetailsAvailableDiscountItem instanceof \Travelport\UniversalRecord\StructType\AvailableDiscount) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AvailableDiscount property can only contain items of type \Travelport\UniversalRecord\StructType\AvailableDiscount, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: array
        if ('' !== ($availableDiscountArrayErrorMessage = self::validateAvailableDiscountForArrayConstraintsFromSetAvailableDiscount($availableDiscount))) {
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\UniversalRecord\StructType\AvailableDiscount $item
     * @return \Travelport\UniversalRecord\StructType\BagDetails
     */
    public function addToAvailableDiscount(\Travelport\UniversalRecord\StructType\AvailableDiscount $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\AvailableDiscount) {
            throw new InvalidArgumentException(sprintf('The AvailableDiscount property can only contain items of type \Travelport\UniversalRecord\StructType\AvailableDiscount, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
