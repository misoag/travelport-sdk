...
class Enumeration extends AbstractStructBase
{
    ...
    public static function validateSolutionGroupForArrayConstraintsFromSetSolutionGroup(?array $values = []): string
    {
        ...
        foreach ($values as $enumerationSolutionGroupItem) {
            ...
            if (!$enumerationSolutionGroupItem instanceof \Travelport\UniversalRecord\StructType\SolutionGroup) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SolutionGroup property can only contain items of type \Travelport\UniversalRecord\StructType\SolutionGroup, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\SolutionGroup) {
            throw new InvalidArgumentException(sprintf('The SolutionGroup property can only contain items of type \Travelport\UniversalRecord\StructType\SolutionGroup, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
