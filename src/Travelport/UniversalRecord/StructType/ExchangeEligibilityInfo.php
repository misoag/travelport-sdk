...
class ExchangeEligibilityInfo extends AbstractStructBase
{
    ...
    /**
     * This method is responsible for validating the values passed to the setExchangePenaltyInfo method
     ...
     */
    public static function validateExchangePenaltyInfoForArrayConstraintsFromSetExchangePenaltyInfo(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $exchangeEligibilityInfoExchangePenaltyInfoItem) {
            ...
            if (!$exchangeEligibilityInfoExchangePenaltyInfoItem instanceof \Travelport\UniversalRecord\StructType\ExchangePenaltyInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ExchangePenaltyInfo property can only contain items of type \Travelport\UniversalRecord\StructType\ExchangePenaltyInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    public function setExchangePenaltyInfo(?array $exchangePenaltyInfo = null): self
    {
        ...
        if ('' !== ($exchangePenaltyInfoArrayErrorMessage = self::validateExchangePenaltyInfoForArrayConstraintsFromSetExchangePenaltyInfo($exchangePenaltyInfo))) {
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\ExchangePenaltyInfo) {
            throw new InvalidArgumentException(sprintf('The ExchangePenaltyInfo property can only contain items of type \Travelport\UniversalRecord\StructType\ExchangePenaltyInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
