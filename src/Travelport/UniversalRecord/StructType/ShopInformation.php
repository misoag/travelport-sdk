...
class ShopInformation extends AbstractStructBase
{
    ...
    public static function validateSearchRequestForArrayConstraintsFromSetSearchRequest(?array $values = []): string
    {
        ...
        foreach ($values as $shopInformationSearchRequestItem) {
            ...
            if (!$shopInformationSearchRequestItem instanceof \Travelport\UniversalRecord\StructType\SearchRequest) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SearchRequest property can only contain items of type \Travelport\UniversalRecord\StructType\SearchRequest, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\SearchRequest) {
            throw new InvalidArgumentException(sprintf('The SearchRequest property can only contain items of type \Travelport\UniversalRecord\StructType\SearchRequest, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateFlightsOfferedForArrayConstraintsFromSetFlightsOffered(?array $values = []): string
    {
        ...
        foreach ($values as $shopInformationFlightsOfferedItem) {
            // validation for constraint: itemType
            if (!$shopInformationFlightsOfferedItem instanceof \Travelport\UniversalRecord\StructType\FlightsOffered) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The FlightsOffered property can only contain items of type \Travelport\UniversalRecord\StructType\FlightsOffered, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\FlightsOffered) {
            throw new InvalidArgumentException(sprintf('The FlightsOffered property can only contain items of type \Travelport\UniversalRecord\StructType\FlightsOffered, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
