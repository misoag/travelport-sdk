...
class QueuePlace extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: QueueSelector
     * @var \Travelport\UniversalRecord\StructType\QueueSelector[]
     */
     ...
     * @param \Travelport\UniversalRecord\StructType\QueueSelector[] $queueSelector
     */
     ...
     * @param string $pseudoCityCode
     * @return \Travelport\UniversalRecord\StructType\QueuePlace
     */
     ...
     * @return \Travelport\UniversalRecord\StructType\QueueSelector[]
     */
    ...
    /**
     * This method is responsible for validating the values passed to the setQueueSelector method
     ...
     */
    public static function validateQueueSelectorForArrayConstraintsFromSetQueueSelector(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $queuePlaceQueueSelectorItem) {
            ...
            if (!$queuePlaceQueueSelectorItem instanceof \Travelport\UniversalRecord\StructType\QueueSelector) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The QueueSelector property can only contain items of type \Travelport\UniversalRecord\StructType\QueueSelector, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    public function setQueueSelector(?array $queueSelector = null): self
    {
        ...
        if ('' !== ($queueSelectorArrayErrorMessage = self::validateQueueSelectorForArrayConstraintsFromSetQueueSelector($queueSelector))) {
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\UniversalRecord\StructType\QueueSelector $item
     * @return \Travelport\UniversalRecord\StructType\QueuePlace
     */
    public function addToQueueSelector(\Travelport\UniversalRecord\StructType\QueueSelector $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\QueueSelector) {
            throw new InvalidArgumentException(sprintf('The QueueSelector property can only contain items of type \Travelport\UniversalRecord\StructType\QueueSelector, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
