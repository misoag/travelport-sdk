...
class TicketInfo extends AbstractStructBase
{
     ...
     * @var \Travelport\UniversalRecord\StructType\ConjunctedTicketInfo[]
     */
     ...
     * @param \Travelport\UniversalRecord\StructType\ConjunctedTicketInfo[] $conjunctedTicketInfo
    ...
    public static function validateConjunctedTicketInfoForArrayConstraintFromSetConjunctedTicketInfo(?array $values = []): string
    {
        ...
        foreach ($values as $ticketInfoConjunctedTicketInfoItem) {
            ...
            if (!$ticketInfoConjunctedTicketInfoItem instanceof \Travelport\UniversalRecord\StructType\ConjunctedTicketInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ConjunctedTicketInfo property can only contain items of type \Travelport\UniversalRecord\StructType\ConjunctedTicketInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\UniversalRecord\StructType\ConjunctedTicketInfo $item
     * @return \Travelport\UniversalRecord\StructType\TicketInfo
     */
    public function addToConjunctedTicketInfo(\Travelport\UniversalRecord\StructType\ConjunctedTicketInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\ConjunctedTicketInfo) {
            throw new InvalidArgumentException(sprintf('The ConjunctedTicketInfo property can only contain items of type \Travelport\UniversalRecord\StructType\ConjunctedTicketInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(3)
    ...
    public static function validateExchangedTicketInfoForArrayConstraintFromSetExchangedTicketInfo(?array $values = []): string
    {
        ...
        foreach ($values as $ticketInfoExchangedTicketInfoItem) {
            // validation for constraint: itemType
            if (!$ticketInfoExchangedTicketInfoItem instanceof \Travelport\UniversalRecord\StructType\ExchangedTicketInfo) {
