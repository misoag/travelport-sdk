...
class TypeProviderReservationSpecificInfo extends AbstractStructBase
{
    ...
    public static function validateOperatedByForArrayConstraintsFromSetOperatedBy(?array $values = []): string
    {
        ...
        foreach ($values as $typeProviderReservationSpecificInfoOperatedByItem) {
            ...
            if (!$typeProviderReservationSpecificInfoOperatedByItem instanceof \Travelport\UniversalRecord\StructType\OperatedBy) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The OperatedBy property can only contain items of type \Travelport\UniversalRecord\StructType\OperatedBy, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\OperatedBy) {
            throw new InvalidArgumentException(sprintf('The OperatedBy property can only contain items of type \Travelport\UniversalRecord\StructType\OperatedBy, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
