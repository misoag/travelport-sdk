...
class FareRemark extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: URL
     * @var \Travelport\UniversalRecord\StructType\URL[]
     */
     ...
     * @param string[] $text
     * @param \Travelport\UniversalRecord\StructType\URL[] $uRL
     * @param string $key
     * @param string $name
     */
    ...
    public static function validateURLForArrayConstraintFromSetURL(?array $values = []): string
    {
        ...
        foreach ($values as $fareRemarkURLItem) {
            // validation for constraint: itemType
            if (!$fareRemarkURLItem instanceof \Travelport\UniversalRecord\StructType\URL) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The URL property can only contain items of type \Travelport\UniversalRecord\StructType\URL, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\UniversalRecord\StructType\URL $item
     * @return \Travelport\UniversalRecord\StructType\FareRemark
     */
    public function addToURL(\Travelport\UniversalRecord\StructType\URL $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\URL) {
            throw new InvalidArgumentException(sprintf('The URL property can only contain items of type \Travelport\UniversalRecord\StructType\URL, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
