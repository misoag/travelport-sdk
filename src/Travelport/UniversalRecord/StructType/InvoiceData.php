...
class InvoiceData extends AbstractStructBase
{
    ...
    public static function validateBookingTravelerInformationForArrayConstraintFromSetBookingTravelerInformation(?array $values = []): string
    {
        ...
        foreach ($values as $invoiceDataBookingTravelerInformationItem) {
            ...
            if (!$invoiceDataBookingTravelerInformationItem instanceof \Travelport\UniversalRecord\StructType\BookingTravelerInformation) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The BookingTravelerInformation property can only contain items of type \Travelport\UniversalRecord\StructType\BookingTravelerInformation, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\BookingTravelerInformation) {
            throw new InvalidArgumentException(sprintf('The BookingTravelerInformation property can only contain items of type \Travelport\UniversalRecord\StructType\BookingTravelerInformation, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(9)
