...
class BookingGuestInformation extends AbstractStructBase
{
    ...
    public static function validateRoomForArrayConstraintsFromSetRoom(?array $values = []): string
    {
        ...
        foreach ($values as $bookingGuestInformationRoomItem) {
            ...
            if (!$bookingGuestInformationRoomItem instanceof \Travelport\UniversalRecord\StructType\TypeGuestRoomInformation) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Room property can only contain items of type \Travelport\UniversalRecord\StructType\TypeGuestRoomInformation, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\TypeGuestRoomInformation) {
            throw new InvalidArgumentException(sprintf('The Room property can only contain items of type \Travelport\UniversalRecord\StructType\TypeGuestRoomInformation, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(9)
