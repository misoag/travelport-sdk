...
class HotelProperty extends AbstractStructBase
{
     ...
     */
    protected ?\Travelport\UniversalRecord\StructType\TypeUnstructuredAddress $PropertyAddress = null;
    /**
     * The PhoneNumber
     * Meta information extracted from the WSDL
     * - maxOccurs: 999
     ...
     */
    protected ?\Travelport\UniversalRecord\StructType\Amenities $Amenities = null;
    /**
     ...
     */
    protected ?\Travelport\UniversalRecord\StructType\MarketingMessage $MarketingMessage = null;
    /**
     * The HotelLocation
     * Meta information extracted from the WSDL
    ...
    public static function validatePhoneNumberForArrayConstraintsFromSetPhoneNumber(?array $values = []): string
    {
        ...
        foreach ($values as $hotelPropertyPhoneNumberItem) {
            ...
            if (!$hotelPropertyPhoneNumberItem instanceof \Travelport\UniversalRecord\StructType\PhoneNumber) {
    ...
    public static function validateHotelRatingForArrayConstraintsFromSetHotelRating(?array $values = []): string
    {
        ...
        foreach ($values as $hotelPropertyHotelRatingItem) {
            // validation for constraint: itemType
            if (!$hotelPropertyHotelRatingItem instanceof \Travelport\UniversalRecord\StructType\HotelRating) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The HotelRating property can only contain items of type \Travelport\UniversalRecord\StructType\HotelRating, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\HotelRating) {
            throw new InvalidArgumentException(sprintf('The HotelRating property can only contain items of type \Travelport\UniversalRecord\StructType\HotelRating, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public function setReserveRequirement(?string $reserveRequirement = null): self
    {
        ...
        if (!\Travelport\UniversalRecord\EnumType\TypeReserveRequirement::valueIsValid($reserveRequirement)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\UniversalRecord\EnumType\TypeReserveRequirement', is_array($reserveRequirement) ? implode(', ', $reserveRequirement) : var_export($reserveRequirement, true), implode(', ', \Travelport\UniversalRecord\EnumType\TypeReserveRequirement::getValidValues())), __LINE__);
        }
     ...
     * @uses \Travelport\UniversalRecord\EnumType\TypeHotelAvailability::valueIsValid()
     * @uses \Travelport\UniversalRecord\EnumType\TypeHotelAvailability::getValidValues()
     * @throws InvalidArgumentException
     * @param string $availability
     * @return \Travelport\UniversalRecord\StructType\HotelProperty
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\UniversalRecord\EnumType\TypeHotelAvailability::valueIsValid($availability)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\UniversalRecord\EnumType\TypeHotelAvailability', is_array($availability) ? implode(', ', $availability) : var_export($availability, true), implode(', ', \Travelport\UniversalRecord\EnumType\TypeHotelAvailability::getValidValues())), __LINE__);
        }
     ...
     * @uses \Travelport\UniversalRecord\EnumType\TypeNetTransCommission::valueIsValid()
     * @uses \Travelport\UniversalRecord\EnumType\TypeNetTransCommission::getValidValues()
     * @throws InvalidArgumentException
     * @param string $netTransCommissionInd
     * @return \Travelport\UniversalRecord\StructType\HotelProperty
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\UniversalRecord\EnumType\TypeNetTransCommission::valueIsValid($netTransCommissionInd)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\UniversalRecord\EnumType\TypeNetTransCommission', is_array($netTransCommissionInd) ? implode(', ', $netTransCommissionInd) : var_export($netTransCommissionInd, true), implode(', ', \Travelport\UniversalRecord\EnumType\TypeNetTransCommission::getValidValues())), __LINE__);
        }
