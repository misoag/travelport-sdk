...
class MediaItem extends AbstractStructBase
{
    ...
    public static function validateSizeCodeForUnionConstraintsFromSetSizeCode($value): string
    {
        ...
        if (!\Travelport\UniversalRecord\EnumType\TypeImageSize::valueIsValid($value)) {
            $exception0 = new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\UniversalRecord\EnumType\TypeImageSize', is_array($value) ? implode(', ', $value) : var_export($value, true), implode(', ', \Travelport\UniversalRecord\EnumType\TypeImageSize::getValidValues())), __LINE__);
        }
        // validation for constraint: enumeration
        if (!\Travelport\UniversalRecord\EnumType\TypeOtherImageSize::valueIsValid($value)) {
            $exception1 = new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\UniversalRecord\EnumType\TypeOtherImageSize', is_array($value) ? implode(', ', $value) : var_export($value, true), implode(', ', \Travelport\UniversalRecord\EnumType\TypeOtherImageSize::getValidValues())), __LINE__);
        }
