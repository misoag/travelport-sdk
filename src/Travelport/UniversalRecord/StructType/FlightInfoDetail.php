...
class FlightInfoDetail extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: Variance
     * @var \Travelport\UniversalRecord\StructType\Variance[]
     */
     ...
     * @param \Travelport\UniversalRecord\StructType\Variance[] $variance
     ...
     */
    public function __construct(string $origin, string $destination, ?\Travelport\UniversalRecord\StructType\CodeshareInfo $codeshareInfo = null, ?array $meals = null, ?array $inFlightServices = null, ?array $variance = null, ?string $scheduledDepartureTime = null, ?string $scheduledArrivalTime = null, ?int $travelTime = null, ?string $eTicketability = null, ?string $equipment = null, ?string $originTerminal = null, ?string $originGate = null, ?string $destinationTerminal = null, ?string $destinationGate = null, ?bool $automatedCheckin = false)
    {
        $this
            ->setOrigin($origin)
    ...
    public static function validateMealsForArrayConstraintFromSetMeals(?array $values = []): string
    {
        ...
        foreach ($values as $flightInfoDetailMealsItem) {
            ...
            if (!\Travelport\UniversalRecord\EnumType\TypeMealService::valueIsValid($flightInfoDetailMealsItem)) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\UniversalRecord\EnumType\TypeMealService', is_array($invalidValues) ? implode(', ', $invalidValues) : var_export($invalidValues, true), implode(', ', \Travelport\UniversalRecord\EnumType\TypeMealService::getValidValues()));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\UniversalRecord\EnumType\TypeMealService::valueIsValid($item)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\UniversalRecord\EnumType\TypeMealService', is_array($item) ? implode(', ', $item) : var_export($item, true), implode(', ', \Travelport\UniversalRecord\EnumType\TypeMealService::getValidValues())), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateVarianceForArrayConstraintFromSetVariance(?array $values = []): string
    {
        ...
        foreach ($values as $flightInfoDetailVarianceItem) {
            // validation for constraint: itemType
            if (!$flightInfoDetailVarianceItem instanceof \Travelport\UniversalRecord\StructType\Variance) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Variance property can only contain items of type \Travelport\UniversalRecord\StructType\Variance, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\UniversalRecord\StructType\Variance $item
     * @return \Travelport\UniversalRecord\StructType\FlightInfoDetail
     */
    public function addToVariance(\Travelport\UniversalRecord\StructType\Variance $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\Variance) {
            throw new InvalidArgumentException(sprintf('The Variance property can only contain items of type \Travelport\UniversalRecord\StructType\Variance, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
