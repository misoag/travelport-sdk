...
class TypeKeyword extends AbstractStructBase
{
     ...
     * - maxOccurs: 99
     * - minOccurs: 0
     * @var \Travelport\UniversalRecord\StructType\TypeSubKey[]
     */
     ...
     * @param string $name
     * @param \Travelport\UniversalRecord\StructType\TypeSubKey[] $subKey
     * @param string[] $text
     * @param string $number
    ...
    /**
     * Get SubKey value
     * @return \Travelport\UniversalRecord\StructType\TypeSubKey[]
     */
    ...
    public static function validateSubKeyForArrayConstraintsFromSetSubKey(?array $values = []): string
    {
        ...
        foreach ($values as $typeKeywordSubKeyItem) {
            ...
            if (!$typeKeywordSubKeyItem instanceof \Travelport\UniversalRecord\StructType\TypeSubKey) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SubKey property can only contain items of type \Travelport\UniversalRecord\StructType\TypeSubKey, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\UniversalRecord\StructType\TypeSubKey $item
     * @return \Travelport\UniversalRecord\StructType\TypeKeyword
     */
    public function addToSubKey(\Travelport\UniversalRecord\StructType\TypeSubKey $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\TypeSubKey) {
            throw new InvalidArgumentException(sprintf('The SubKey property can only contain items of type \Travelport\UniversalRecord\StructType\TypeSubKey, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
