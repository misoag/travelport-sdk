...
class SearchPriority extends AbstractStructBase
{
    ...
    public static function validateCriteriaForArrayConstraintFromSetCriteria(?array $values = []): string
    {
        ...
        foreach ($values as $searchPriorityCriteriaItem) {
            ...
            if (!$searchPriorityCriteriaItem instanceof \Travelport\UniversalRecord\StructType\Criteria) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Criteria property can only contain items of type \Travelport\UniversalRecord\StructType\Criteria, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\Criteria) {
            throw new InvalidArgumentException(sprintf('The Criteria property can only contain items of type \Travelport\UniversalRecord\StructType\Criteria, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(8)
