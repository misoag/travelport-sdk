...
class MCOInformation extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: PassengerInfo
     * @var \Travelport\UniversalRecord\StructType\PassengerInfo[]
     */
     ...
     * @param \Travelport\UniversalRecord\StructType\PassengerInfo[] $passengerInfo
     ...
     * @return \Travelport\UniversalRecord\StructType\PassengerInfo[]
     */
    ...
    public static function validatePassengerInfoForArrayConstraintFromSetPassengerInfo(?array $values = []): string
    {
        ...
        foreach ($values as $mCOInformationPassengerInfoItem) {
            ...
            if (!$mCOInformationPassengerInfoItem instanceof \Travelport\UniversalRecord\StructType\PassengerInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PassengerInfo property can only contain items of type \Travelport\UniversalRecord\StructType\PassengerInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\UniversalRecord\StructType\PassengerInfo $item
     * @return \Travelport\UniversalRecord\StructType\MCOInformation
     */
    public function addToPassengerInfo(\Travelport\UniversalRecord\StructType\PassengerInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\PassengerInfo) {
            throw new InvalidArgumentException(sprintf('The PassengerInfo property can only contain items of type \Travelport\UniversalRecord\StructType\PassengerInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
