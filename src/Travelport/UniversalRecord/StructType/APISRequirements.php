...
class APISRequirements extends AbstractStructBase
{
    ...
    public static function validateDocumentForArrayConstraintFromSetDocument(?array $values = []): string
    {
        ...
        foreach ($values as $aPISRequirementsDocumentItem) {
            ...
            if (!$aPISRequirementsDocumentItem instanceof \Travelport\UniversalRecord\StructType\Document) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Document property can only contain items of type \Travelport\UniversalRecord\StructType\Document, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\Document) {
            throw new InvalidArgumentException(sprintf('The Document property can only contain items of type \Travelport\UniversalRecord\StructType\Document, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
