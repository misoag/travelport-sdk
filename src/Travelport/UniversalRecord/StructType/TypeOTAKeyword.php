...
class TypeOTAKeyword extends AbstractStructBase
{
    ...
    public static function validateSubKeyForArrayConstraintsFromSetSubKey(?array $values = []): string
    {
        ...
        foreach ($values as $typeOTAKeywordSubKeyItem) {
            ...
            if (!$typeOTAKeywordSubKeyItem instanceof \Travelport\UniversalRecord\StructType\TypeOTASubKey) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SubKey property can only contain items of type \Travelport\UniversalRecord\StructType\TypeOTASubKey, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\TypeOTASubKey) {
            throw new InvalidArgumentException(sprintf('The SubKey property can only contain items of type \Travelport\UniversalRecord\StructType\TypeOTASubKey, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
