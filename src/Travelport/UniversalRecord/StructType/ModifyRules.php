...
class ModifyRules extends AbstractStructBase
{
     ...
     * Meta information extracted from the WSDL
     * - maxOccurs: 999
     * @var \Travelport\UniversalRecord\StructType\ModifyRule[]
     */
     ...
     * @param \Travelport\UniversalRecord\StructType\ModifyRule[] $modifyRule
    ...
    public static function validateModifyRuleForArrayConstraintFromSetModifyRule(?array $values = []): string
    {
        ...
        foreach ($values as $modifyRulesModifyRuleItem) {
            ...
            if (!$modifyRulesModifyRuleItem instanceof \Travelport\UniversalRecord\StructType\ModifyRule) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ModifyRule property can only contain items of type \Travelport\UniversalRecord\StructType\ModifyRule, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\UniversalRecord\StructType\ModifyRule[] $modifyRule
     * @return \Travelport\UniversalRecord\StructType\ModifyRules
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\UniversalRecord\StructType\ModifyRule $item
     * @return \Travelport\UniversalRecord\StructType\ModifyRules
     */
    public function addToModifyRule(\Travelport\UniversalRecord\StructType\ModifyRule $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\ModifyRule) {
            throw new InvalidArgumentException(sprintf('The ModifyRule property can only contain items of type \Travelport\UniversalRecord\StructType\ModifyRule, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateSupportedModificationsForArrayConstraintFromSetSupportedModifications(?array $values = []): string
    {
        ...
        foreach ($values as $modifyRulesSupportedModificationsItem) {
            ...
            if (!\Travelport\UniversalRecord\EnumType\ModificationType::valueIsValid($modifyRulesSupportedModificationsItem)) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\UniversalRecord\EnumType\ModificationType', is_array($invalidValues) ? implode(', ', $invalidValues) : var_export($invalidValues, true), implode(', ', \Travelport\UniversalRecord\EnumType\ModificationType::getValidValues()));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\UniversalRecord\EnumType\ModificationType::valueIsValid($item)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\UniversalRecord\EnumType\ModificationType', is_array($item) ? implode(', ', $item) : var_export($item, true), implode(', ', \Travelport\UniversalRecord\EnumType\ModificationType::getValidValues())), __LINE__);
        }
