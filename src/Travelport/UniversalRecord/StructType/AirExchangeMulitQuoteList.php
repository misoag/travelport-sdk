...
class AirExchangeMulitQuoteList extends AbstractStructBase
{
    ...
    /**
     * This method is responsible for validating the values passed to the setAirExchangeMultiQuoteOption method
     ...
     */
    public static function validateAirExchangeMultiQuoteOptionForArrayConstraintsFromSetAirExchangeMultiQuoteOption(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $airExchangeMulitQuoteListAirExchangeMultiQuoteOptionItem) {
            ...
            if (!$airExchangeMulitQuoteListAirExchangeMultiQuoteOptionItem instanceof \Travelport\UniversalRecord\StructType\AirExchangeMultiQuoteOption) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AirExchangeMultiQuoteOption property can only contain items of type \Travelport\UniversalRecord\StructType\AirExchangeMultiQuoteOption, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    public function setAirExchangeMultiQuoteOption(?array $airExchangeMultiQuoteOption = null): self
    {
        ...
        if ('' !== ($airExchangeMultiQuoteOptionArrayErrorMessage = self::validateAirExchangeMultiQuoteOptionForArrayConstraintsFromSetAirExchangeMultiQuoteOption($airExchangeMultiQuoteOption))) {
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\AirExchangeMultiQuoteOption) {
            throw new InvalidArgumentException(sprintf('The AirExchangeMultiQuoteOption property can only contain items of type \Travelport\UniversalRecord\StructType\AirExchangeMultiQuoteOption, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
