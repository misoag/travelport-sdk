...
class SeatAttributes extends AbstractStructBase
{
    ...
    public static function validateSeatAttributeForArrayConstraintFromSetSeatAttribute(?array $values = []): string
    {
        ...
        foreach ($values as $seatAttributesSeatAttributeItem) {
            ...
            if (!$seatAttributesSeatAttributeItem instanceof \Travelport\UniversalRecord\StructType\SeatAttribute) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SeatAttribute property can only contain items of type \Travelport\UniversalRecord\StructType\SeatAttribute, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\SeatAttribute) {
            throw new InvalidArgumentException(sprintf('The SeatAttribute property can only contain items of type \Travelport\UniversalRecord\StructType\SeatAttribute, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(10)
