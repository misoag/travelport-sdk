...
class Coupon extends AbstractStructBase
{
     ...
     * @var \Travelport\UniversalRecord\StructType\TicketDesignator[]
     */
     ...
     * @param string $status
     * @param \Travelport\UniversalRecord\StructType\TicketDesignator[] $ticketDesignator
     * @param string $key
     ...
     * @return \Travelport\UniversalRecord\StructType\TicketDesignator[]
     */
    ...
    /**
     * This method is responsible for validating the value(s) passed to the setTicketDesignator method
     ...
     */
    public static function validateTicketDesignatorForArrayConstraintFromSetTicketDesignator(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $couponTicketDesignatorItem) {
            ...
            if (!$couponTicketDesignatorItem instanceof \Travelport\UniversalRecord\StructType\TicketDesignator) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The TicketDesignator property can only contain items of type \Travelport\UniversalRecord\StructType\TicketDesignator, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    public function setTicketDesignator(?array $ticketDesignator = null): self
    {
        ...
        if ('' !== ($ticketDesignatorArrayErrorMessage = self::validateTicketDesignatorForArrayConstraintFromSetTicketDesignator($ticketDesignator))) {
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\UniversalRecord\StructType\TicketDesignator $item
     * @return \Travelport\UniversalRecord\StructType\Coupon
     */
    public function addToTicketDesignator(\Travelport\UniversalRecord\StructType\TicketDesignator $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\TicketDesignator) {
            throw new InvalidArgumentException(sprintf('The TicketDesignator property can only contain items of type \Travelport\UniversalRecord\StructType\TicketDesignator, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
