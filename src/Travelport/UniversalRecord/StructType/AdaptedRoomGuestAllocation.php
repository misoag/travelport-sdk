...
class AdaptedRoomGuestAllocation extends AbstractStructBase
{
    ...
    public static function validateRoomForArrayConstraintsFromSetRoom(?array $values = []): string
    {
        ...
        foreach ($values as $adaptedRoomGuestAllocationRoomItem) {
            ...
            if (!$adaptedRoomGuestAllocationRoomItem instanceof \Travelport\UniversalRecord\StructType\TypeAdaptedRoomGuestAllocation) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Room property can only contain items of type \Travelport\UniversalRecord\StructType\TypeAdaptedRoomGuestAllocation, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\TypeAdaptedRoomGuestAllocation) {
            throw new InvalidArgumentException(sprintf('The Room property can only contain items of type \Travelport\UniversalRecord\StructType\TypeAdaptedRoomGuestAllocation, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(9)
