...
class SecondaryTypeRule extends AbstractStructBase
{
    ...
    public static function validateApplicationLimitForArrayConstraintFromSetApplicationLimit(?array $values = []): string
    {
        ...
        foreach ($values as $secondaryTypeRuleApplicationLimitItem) {
            ...
            if (!$secondaryTypeRuleApplicationLimitItem instanceof \Travelport\UniversalRecord\StructType\OptionalServiceApplicationLimitType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ApplicationLimit property can only contain items of type \Travelport\UniversalRecord\StructType\OptionalServiceApplicationLimitType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\OptionalServiceApplicationLimitType) {
            throw new InvalidArgumentException(sprintf('The ApplicationLimit property can only contain items of type \Travelport\UniversalRecord\StructType\OptionalServiceApplicationLimitType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(10)
