...
class Segment extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: SegmentRemark
     * @var \Travelport\UniversalRecord\StructType\SegmentRemark[]
     */
     ...
     * @param string $key
     * @param \Travelport\UniversalRecord\StructType\SegmentRemark[] $segmentRemark
     * @param string $status
     * @param bool $passive
     ...
     * Set Key value
     * @param string $key
     * @return \Travelport\UniversalRecord\StructType\Segment
     */
     ...
     * @return \Travelport\UniversalRecord\StructType\SegmentRemark[]
     */
    ...
    public static function validateSegmentRemarkForArrayConstraintsFromSetSegmentRemark(?array $values = []): string
    {
        ...
        foreach ($values as $segmentSegmentRemarkItem) {
            ...
            if (!$segmentSegmentRemarkItem instanceof \Travelport\UniversalRecord\StructType\SegmentRemark) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SegmentRemark property can only contain items of type \Travelport\UniversalRecord\StructType\SegmentRemark, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\UniversalRecord\StructType\SegmentRemark $item
     * @return \Travelport\UniversalRecord\StructType\Segment
     */
    public function addToSegmentRemark(\Travelport\UniversalRecord\StructType\SegmentRemark $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\SegmentRemark) {
            throw new InvalidArgumentException(sprintf('The SegmentRemark property can only contain items of type \Travelport\UniversalRecord\StructType\SegmentRemark, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
