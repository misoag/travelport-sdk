...
class TypeTaxInfo extends AbstractStructBase
{
    ...
    public static function validateTaxDetailForArrayConstraintsFromSetTaxDetail(?array $values = []): string
    {
        ...
        foreach ($values as $typeTaxInfoTaxDetailItem) {
            ...
            if (!$typeTaxInfoTaxDetailItem instanceof \Travelport\UniversalRecord\StructType\TaxDetail) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The TaxDetail property can only contain items of type \Travelport\UniversalRecord\StructType\TaxDetail, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\TaxDetail) {
            throw new InvalidArgumentException(sprintf('The TaxDetail property can only contain items of type \Travelport\UniversalRecord\StructType\TaxDetail, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
