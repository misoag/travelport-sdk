...
class TypeAdaptedRoomGuestAllocation extends AbstractStructBase
{
    ...
    public static function validateChildForArrayConstraintsFromSetChild(?array $values = []): string
    {
        ...
        foreach ($values as $typeAdaptedRoomGuestAllocationChildItem) {
            ...
            if (!$typeAdaptedRoomGuestAllocationChildItem instanceof \Travelport\UniversalRecord\StructType\TypeGuestChildInformation) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Child property can only contain items of type \Travelport\UniversalRecord\StructType\TypeGuestChildInformation, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\TypeGuestChildInformation) {
            throw new InvalidArgumentException(sprintf('The Child property can only contain items of type \Travelport\UniversalRecord\StructType\TypeGuestChildInformation, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(6)
