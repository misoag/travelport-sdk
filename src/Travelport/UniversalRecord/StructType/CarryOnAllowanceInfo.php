...
class CarryOnAllowanceInfo extends BaseBaggageAllowanceInfo
{
    ...
    public static function validateCarryOnDetailsForArrayConstraintFromSetCarryOnDetails(?array $values = []): string
    {
        ...
        foreach ($values as $carryOnAllowanceInfoCarryOnDetailsItem) {
            ...
            if (!$carryOnAllowanceInfoCarryOnDetailsItem instanceof \Travelport\UniversalRecord\StructType\CarryOnDetails) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The CarryOnDetails property can only contain items of type \Travelport\UniversalRecord\StructType\CarryOnDetails, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\UniversalRecord\StructType\CarryOnDetails) {
            throw new InvalidArgumentException(sprintf('The CarryOnDetails property can only contain items of type \Travelport\UniversalRecord\StructType\CarryOnDetails, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
