...
class TotalPenaltyTaxInfo extends AbstractStructBase
{
    ...
    public static function validatePenaltyTaxInfoForArrayConstraintsFromSetPenaltyTaxInfo(?array $values = []): string
    {
        ...
        foreach ($values as $totalPenaltyTaxInfoPenaltyTaxInfoItem) {
            ...
            if (!$totalPenaltyTaxInfoPenaltyTaxInfoItem instanceof \Travelport\Hotel\StructType\TypeTax) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PenaltyTaxInfo property can only contain items of type \Travelport\Hotel\StructType\TypeTax, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    public function addToPenaltyTaxInfo(\Travelport\Hotel\StructType\TypeTax $item): self
    {
        ...
        if (!$item instanceof \Travelport\Hotel\StructType\TypeTax) {
            throw new InvalidArgumentException(sprintf('The PenaltyTaxInfo property can only contain items of type \Travelport\Hotel\StructType\TypeTax, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
