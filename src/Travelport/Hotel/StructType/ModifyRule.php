...
class ModifyRule extends AbstractStructBase
{
    ...
    public function setModification(string $modification): self
    {
        ...
        if (!\Travelport\Hotel\EnumType\ModificationType::valueIsValid($modification)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Hotel\EnumType\ModificationType', is_array($modification) ? implode(', ', $modification) : var_export($modification, true), implode(', ', \Travelport\Hotel\EnumType\ModificationType::getValidValues())), __LINE__);
        }
