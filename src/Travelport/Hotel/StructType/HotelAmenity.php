...
class HotelAmenity extends AbstractStructBase
{
    ...
    /**
     * Set Level value
     * @uses \Travelport\Hotel\EnumType\TypeAmenityLevel::valueIsValid()
     * @uses \Travelport\Hotel\EnumType\TypeAmenityLevel::getValidValues()
     * @throws InvalidArgumentException
     * @param string $level
     * @return \Travelport\Hotel\StructType\HotelAmenity
     */
    public function setLevel(string $level): self
    {
        // validation for constraint: enumeration
        if (!\Travelport\Hotel\EnumType\TypeAmenityLevel::valueIsValid($level)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Hotel\EnumType\TypeAmenityLevel', is_array($level) ? implode(', ', $level) : var_export($level, true), implode(', ', \Travelport\Hotel\EnumType\TypeAmenityLevel::getValidValues())), __LINE__);
        }
        $this->Level = $level;
