...
class APIProvider extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: TransactionType
     * @var \Travelport\Hotel\StructType\TransactionType|null
     */
    protected ?\Travelport\Hotel\StructType\TransactionType $TransactionType = null;
    /**
     * The AvailablePseudoCityCode
     * Meta information extracted from the WSDL
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \Travelport\Hotel\StructType\AvailablePseudoCityCode[]
     */
     ...
     * @param \Travelport\Hotel\StructType\TransactionType $transactionType
     * @param \Travelport\Hotel\StructType\AvailablePseudoCityCode[] $availablePseudoCityCode
     * @param string $supplierCode
     * @param string $iATACode
     */
    public function __construct(string $providerCode, ?\Travelport\Hotel\StructType\TransactionType $transactionType = null, ?array $availablePseudoCityCode = null, ?string $supplierCode = null, ?string $iATACode = null)
    {
        $this
     ...
     * @param string $providerCode
     * @return \Travelport\Hotel\StructType\APIProvider
     */
     ...
     * @return \Travelport\Hotel\StructType\TransactionType|null
     */
    public function getTransactionType(): ?\Travelport\Hotel\StructType\TransactionType
    {
     ...
     * @return \Travelport\Hotel\StructType\APIProvider
     */
    public function setTransactionType(?\Travelport\Hotel\StructType\TransactionType $transactionType = null): self
    {
    ...
    public static function validateAvailablePseudoCityCodeForArrayConstraintsFromSetAvailablePseudoCityCode(?array $values = []): string
    {
        ...
        foreach ($values as $aPIProviderAvailablePseudoCityCodeItem) {
            ...
            if (!$aPIProviderAvailablePseudoCityCodeItem instanceof \Travelport\Hotel\StructType\AvailablePseudoCityCode) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AvailablePseudoCityCode property can only contain items of type \Travelport\Hotel\StructType\AvailablePseudoCityCode, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Hotel\StructType\AvailablePseudoCityCode $item
     * @return \Travelport\Hotel\StructType\APIProvider
     */
    public function addToAvailablePseudoCityCode(\Travelport\Hotel\StructType\AvailablePseudoCityCode $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Hotel\StructType\AvailablePseudoCityCode) {
            throw new InvalidArgumentException(sprintf('The AvailablePseudoCityCode property can only contain items of type \Travelport\Hotel\StructType\AvailablePseudoCityCode, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
