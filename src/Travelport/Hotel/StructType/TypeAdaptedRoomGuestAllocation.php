...
class TypeAdaptedRoomGuestAllocation extends AbstractStructBase
{
     ...
     * - maxOccurs: 6
     * - minOccurs: 0
     * @var \Travelport\Hotel\StructType\TypeGuestChildInformation[]
     */
     ...
     * @param \Travelport\Hotel\StructType\TypeGuestChildInformation[] $child
    ...
    /**
     * Get Child value
     * @return \Travelport\Hotel\StructType\TypeGuestChildInformation[]
     */
    ...
    public static function validateChildForArrayConstraintFromSetChild(?array $values = []): string
    {
        ...
        foreach ($values as $typeAdaptedRoomGuestAllocationChildItem) {
            ...
            if (!$typeAdaptedRoomGuestAllocationChildItem instanceof \Travelport\Hotel\StructType\TypeGuestChildInformation) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Child property can only contain items of type \Travelport\Hotel\StructType\TypeGuestChildInformation, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Hotel\StructType\TypeGuestChildInformation $item
     * @return \Travelport\Hotel\StructType\TypeAdaptedRoomGuestAllocation
     */
    public function addToChild(\Travelport\Hotel\StructType\TypeGuestChildInformation $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Hotel\StructType\TypeGuestChildInformation) {
            throw new InvalidArgumentException(sprintf('The Child property can only contain items of type \Travelport\Hotel\StructType\TypeGuestChildInformation, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(6)
