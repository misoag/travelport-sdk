...
class TypeKeyword extends AbstractStructBase
{
    ...
    public static function validateSubKeyForArrayConstraintsFromSetSubKey(?array $values = []): string
    {
        ...
        foreach ($values as $typeKeywordSubKeyItem) {
            ...
            if (!$typeKeywordSubKeyItem instanceof \Travelport\Hotel\StructType\TypeSubKey) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SubKey property can only contain items of type \Travelport\Hotel\StructType\TypeSubKey, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Hotel\StructType\TypeSubKey) {
            throw new InvalidArgumentException(sprintf('The SubKey property can only contain items of type \Travelport\Hotel\StructType\TypeSubKey, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
