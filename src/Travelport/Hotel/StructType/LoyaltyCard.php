...
class LoyaltyCard extends AbstractStructBase
{
    ...
    public static function validateProviderReservationSpecificInfoForArrayConstraintsFromSetProviderReservationSpecificInfo(?array $values = []): string
    {
        ...
        foreach ($values as $loyaltyCardProviderReservationSpecificInfoItem) {
            ...
            if (!$loyaltyCardProviderReservationSpecificInfoItem instanceof \Travelport\Hotel\StructType\TypeProviderReservationSpecificInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ProviderReservationSpecificInfo property can only contain items of type \Travelport\Hotel\StructType\TypeProviderReservationSpecificInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Hotel\StructType\TypeProviderReservationSpecificInfo) {
            throw new InvalidArgumentException(sprintf('The ProviderReservationSpecificInfo property can only contain items of type \Travelport\Hotel\StructType\TypeProviderReservationSpecificInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
