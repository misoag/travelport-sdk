...
class AddressRestriction extends AbstractStructBase
{
    ...
    public static function validateRequiredFieldForArrayConstraintsFromSetRequiredField(?array $values = []): string
    {
        ...
        foreach ($values as $addressRestrictionRequiredFieldItem) {
            ...
            if (!$addressRestrictionRequiredFieldItem instanceof \Travelport\Hotel\StructType\RequiredField) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The RequiredField property can only contain items of type \Travelport\Hotel\StructType\RequiredField, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Hotel\StructType\RequiredField) {
            throw new InvalidArgumentException(sprintf('The RequiredField property can only contain items of type \Travelport\Hotel\StructType\RequiredField, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
