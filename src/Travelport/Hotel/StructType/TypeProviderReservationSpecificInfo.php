...
class TypeProviderReservationSpecificInfo extends AbstractStructBase
{
     ...
     */
    protected ?\Travelport\Hotel\StructType\ProviderReservationInfoRef $ProviderReservationInfoRef = null;
    /**
    ...
    public static function validateOperatedByForArrayConstraintsFromSetOperatedBy(?array $values = []): string
    {
        ...
        foreach ($values as $typeProviderReservationSpecificInfoOperatedByItem) {
            ...
            if (!$typeProviderReservationSpecificInfoOperatedByItem instanceof \Travelport\Hotel\StructType\OperatedBy) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The OperatedBy property can only contain items of type \Travelport\Hotel\StructType\OperatedBy, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Hotel\StructType\OperatedBy) {
            throw new InvalidArgumentException(sprintf('The OperatedBy property can only contain items of type \Travelport\Hotel\StructType\OperatedBy, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
