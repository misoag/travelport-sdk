...
class MCOExchangeInfo extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: ExchangedCoupon
     * @var \Travelport\Hotel\StructType\ExchangedCoupon[]
     */
     ...
     * @param \Travelport\Hotel\StructType\ExchangedCoupon[] $exchangedCoupon
     * @param string $originalTicketNumber
     * @param string $originalCityCode
     * @param string $originalTicketDate
     * @param string $iATACode
     */
    public function __construct(?\Travelport\Hotel\StructType\FormOfPayment $formOfPayment = null, ?array $exchangedCoupon = null, ?string $originalTicketNumber = null, ?string $originalCityCode = null, ?string $originalTicketDate = null, ?string $iATACode = null)
    {
        $this
     ...
     * @param \Travelport\Hotel\StructType\FormOfPayment $formOfPayment
     * @return \Travelport\Hotel\StructType\MCOExchangeInfo
     */
     ...
     * @return \Travelport\Hotel\StructType\ExchangedCoupon[]
     */
    ...
    public static function validateExchangedCouponForArrayConstraintFromSetExchangedCoupon(?array $values = []): string
    {
        ...
        foreach ($values as $mCOExchangeInfoExchangedCouponItem) {
            ...
            if (!$mCOExchangeInfoExchangedCouponItem instanceof \Travelport\Hotel\StructType\ExchangedCoupon) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ExchangedCoupon property can only contain items of type \Travelport\Hotel\StructType\ExchangedCoupon, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Hotel\StructType\ExchangedCoupon $item
     * @return \Travelport\Hotel\StructType\MCOExchangeInfo
     */
    public function addToExchangedCoupon(\Travelport\Hotel\StructType\ExchangedCoupon $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Hotel\StructType\ExchangedCoupon) {
            throw new InvalidArgumentException(sprintf('The ExchangedCoupon property can only contain items of type \Travelport\Hotel\StructType\ExchangedCoupon, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(4)
