...
class TypeOTAKeyword extends AbstractStructBase
{
     ...
     * - maxOccurs: 99
     * - minOccurs: 0
     * @var \Travelport\Hotel\StructType\TypeOTASubKey[]
     */
     ...
     * @param string $name
     * @param \Travelport\Hotel\StructType\TypeOTASubKey[] $subKey
     * @param string[] $text
     * @param string $number
     ...
     * Set Name value
     * @param string $name
     * @return \Travelport\Hotel\StructType\TypeOTAKeyword
     */
    ...
    /**
     * Get SubKey value
     * @return \Travelport\Hotel\StructType\TypeOTASubKey[]
     */
    ...
    public static function validateSubKeyForArrayConstraintFromSetSubKey(?array $values = []): string
    {
        ...
        foreach ($values as $typeOTAKeywordSubKeyItem) {
            ...
            if (!$typeOTAKeywordSubKeyItem instanceof \Travelport\Hotel\StructType\TypeOTASubKey) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SubKey property can only contain items of type \Travelport\Hotel\StructType\TypeOTASubKey, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Hotel\StructType\TypeOTASubKey $item
     * @return \Travelport\Hotel\StructType\TypeOTAKeyword
     */
    public function addToSubKey(\Travelport\Hotel\StructType\TypeOTASubKey $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Hotel\StructType\TypeOTASubKey) {
            throw new InvalidArgumentException(sprintf('The SubKey property can only contain items of type \Travelport\Hotel\StructType\TypeOTASubKey, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
