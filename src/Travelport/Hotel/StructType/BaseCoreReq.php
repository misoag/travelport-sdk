...
class BaseCoreReq extends AbstractStructBase
{
     ...
     */
    protected ?\Travelport\Hotel\StructType\BillingPointOfSaleInfo $BillingPointOfSaleInfo = null;
    /**
     * The AgentIDOverride
     * Meta information extracted from the WSDL
     * - maxOccurs: 999
     ...
     * Canadian) and PT-BR (Portuguese Brazil) can also be used. For RCH, language codes ENGB, ENUS, DEDE, DECH can also be used. Only certain services support this attribute. Providers: ACH, RCH, 1G, 1V, 1P, 1J.
     * - use: optional
     * @var string|null
     */
    ...
    public static function validateAgentIDOverrideForArrayConstraintsFromSetAgentIDOverride(?array $values = []): string
    {
        ...
        foreach ($values as $baseCoreReqAgentIDOverrideItem) {
            ...
            if (!$baseCoreReqAgentIDOverrideItem instanceof \Travelport\Hotel\StructType\AgentIDOverride) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AgentIDOverride property can only contain items of type \Travelport\Hotel\StructType\AgentIDOverride, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Hotel\StructType\AgentIDOverride) {
            throw new InvalidArgumentException(sprintf('The AgentIDOverride property can only contain items of type \Travelport\Hotel\StructType\AgentIDOverride, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
     ...
     * @uses \Travelport\Hotel\EnumType\TypeLoggingLevel::valueIsValid()
     * @uses \Travelport\Hotel\EnumType\TypeLoggingLevel::getValidValues()
     * @throws InvalidArgumentException
     * @param string $overrideLogging
     * @return \Travelport\Hotel\StructType\BaseCoreReq
     */
    public function setOverrideLogging(?string $overrideLogging = null): self
    {
        ...
        if (!\Travelport\Hotel\EnumType\TypeLoggingLevel::valueIsValid($overrideLogging)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Hotel\EnumType\TypeLoggingLevel', is_array($overrideLogging) ? implode(', ', $overrideLogging) : var_export($overrideLogging, true), implode(', ', \Travelport\Hotel\EnumType\TypeLoggingLevel::getValidValues())), __LINE__);
        }
