...
class PreferredSupplier extends AbstractStructBase
{
     ...
     * @uses \Travelport\Hotel\EnumType\TypeProfileType::valueIsValid()
     * @uses \Travelport\Hotel\EnumType\TypeProfileType::getValidValues()
    ...
    public function setProfileType(string $profileType): self
    {
        ...
        if (!\Travelport\Hotel\EnumType\TypeProfileType::valueIsValid($profileType)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Hotel\EnumType\TypeProfileType', is_array($profileType) ? implode(', ', $profileType) : var_export($profileType, true), implode(', ', \Travelport\Hotel\EnumType\TypeProfileType::getValidValues())), __LINE__);
        }
