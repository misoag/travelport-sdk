...
class ApplicationLevel extends AbstractStructBase
{
     ...
     */
    protected ?\Travelport\Hotel\StructType\ApplicationLimits $ApplicationLimits = null;
    /**
     * The ServiceData
     * Meta information extracted from the WSDL
     * - maxOccurs: 999
    ...
    public static function validateServiceDataForArrayConstraintsFromSetServiceData(?array $values = []): string
    {
        ...
        foreach ($values as $applicationLevelServiceDataItem) {
            ...
            if (!$applicationLevelServiceDataItem instanceof \Travelport\Hotel\StructType\ServiceData) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ServiceData property can only contain items of type \Travelport\Hotel\StructType\ServiceData, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Hotel\StructType\ServiceData) {
            throw new InvalidArgumentException(sprintf('The ServiceData property can only contain items of type \Travelport\Hotel\StructType\ServiceData, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateApplicableLevelsForArrayConstraintsFromSetApplicableLevels(?array $values = []): string
    {
        ...
        foreach ($values as $applicationLevelApplicableLevelsItem) {
            ...
            if (!\Travelport\Hotel\EnumType\OptionalServiceApplicabilityType::valueIsValid($applicationLevelApplicableLevelsItem)) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Hotel\EnumType\OptionalServiceApplicabilityType', is_array($invalidValues) ? implode(', ', $invalidValues) : var_export($invalidValues, true), implode(', ', \Travelport\Hotel\EnumType\OptionalServiceApplicabilityType::getValidValues()));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\Hotel\EnumType\OptionalServiceApplicabilityType::valueIsValid($item)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Hotel\EnumType\OptionalServiceApplicabilityType', is_array($item) ? implode(', ', $item) : var_export($item, true), implode(', ', \Travelport\Hotel\EnumType\OptionalServiceApplicabilityType::getValidValues())), __LINE__);
        }
