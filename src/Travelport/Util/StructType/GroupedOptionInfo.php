...
class GroupedOptionInfo extends AbstractStructBase
{
     ...
     * - minOccurs: 2
     * - ref: GroupedOption
     * @var \Travelport\Util\StructType\GroupedOption[]
     */
    ...
    public static function validateGroupedOptionForArrayConstraintsFromSetGroupedOption(?array $values = []): string
    {
        ...
        foreach ($values as $groupedOptionInfoGroupedOptionItem) {
            ...
            if (!$groupedOptionInfoGroupedOptionItem instanceof \Travelport\Util\StructType\GroupedOption) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The GroupedOption property can only contain items of type \Travelport\Util\StructType\GroupedOption, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\GroupedOption $item
     * @return \Travelport\Util\StructType\GroupedOptionInfo
     */
    public function addToGroupedOption(\Travelport\Util\StructType\GroupedOption $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\GroupedOption) {
