...
class InvoiceData extends AbstractStructBase
{
     ...
     * @var \Travelport\Util\StructType\BookingTravelerInformation[]
     */
     ...
     * @param \Travelport\Util\StructType\BookingTravelerInformation[] $bookingTravelerInformation
     * @param string $key
     ...
     * @param string $invoiceNumber
     * @return \Travelport\Util\StructType\InvoiceData
     */
    ...
    public static function validateBookingTravelerInformationForArrayConstraintsFromSetBookingTravelerInformation(?array $values = []): string
    {
        ...
        foreach ($values as $invoiceDataBookingTravelerInformationItem) {
            ...
            if (!$invoiceDataBookingTravelerInformationItem instanceof \Travelport\Util\StructType\BookingTravelerInformation) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The BookingTravelerInformation property can only contain items of type \Travelport\Util\StructType\BookingTravelerInformation, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\BookingTravelerInformation $item
     * @return \Travelport\Util\StructType\InvoiceData
     */
    public function addToBookingTravelerInformation(\Travelport\Util\StructType\BookingTravelerInformation $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\BookingTravelerInformation) {
            throw new InvalidArgumentException(sprintf('The BookingTravelerInformation property can only contain items of type \Travelport\Util\StructType\BookingTravelerInformation, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(9)
