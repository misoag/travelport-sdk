...
class Routing extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\DirectionInfo[]
     */
     ...
     * @param \Travelport\Util\StructType\DirectionInfo[] $directionInfo
     ...
     * @return \Travelport\Util\StructType\DirectionInfo[]
     */
    ...
    public static function validateDirectionInfoForArrayConstraintFromSetDirectionInfo(?array $values = []): string
    {
        ...
        foreach ($values as $routingDirectionInfoItem) {
            ...
            if (!$routingDirectionInfoItem instanceof \Travelport\Util\StructType\DirectionInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The DirectionInfo property can only contain items of type \Travelport\Util\StructType\DirectionInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\DirectionInfo $item
     * @return \Travelport\Util\StructType\Routing
     */
    public function addToDirectionInfo(\Travelport\Util\StructType\DirectionInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\DirectionInfo) {
            throw new InvalidArgumentException(sprintf('The DirectionInfo property can only contain items of type \Travelport\Util\StructType\DirectionInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
