...
class SeatAttributes extends AbstractStructBase
{
    ...
    public static function validateSeatAttributeForArrayConstraintsFromSetSeatAttribute(?array $values = []): string
    {
        ...
        foreach ($values as $seatAttributesSeatAttributeItem) {
            ...
            if (!$seatAttributesSeatAttributeItem instanceof \Travelport\Util\StructType\SeatAttribute) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SeatAttribute property can only contain items of type \Travelport\Util\StructType\SeatAttribute, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\SeatAttribute[] $seatAttribute
     * @return \Travelport\Util\StructType\SeatAttributes
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\SeatAttribute $item
     * @return \Travelport\Util\StructType\SeatAttributes
     */
    public function addToSeatAttribute(\Travelport\Util\StructType\SeatAttribute $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\SeatAttribute) {
            throw new InvalidArgumentException(sprintf('The SeatAttribute property can only contain items of type \Travelport\Util\StructType\SeatAttribute, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(10)
