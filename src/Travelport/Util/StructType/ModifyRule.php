...
class ModifyRule extends AbstractStructBase
{
    ...
    public function setModification(string $modification): self
    {
        ...
        if (!\Travelport\Util\EnumType\ModificationType::valueIsValid($modification)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\ModificationType', is_array($modification) ? implode(', ', $modification) : var_export($modification, true), implode(', ', \Travelport\Util\EnumType\ModificationType::getValidValues())), __LINE__);
        }
