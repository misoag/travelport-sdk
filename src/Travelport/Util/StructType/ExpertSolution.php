...
class ExpertSolution extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - ref: LegPrice
     * @var \Travelport\Util\StructType\LegPrice[]
     */
     ...
     * @param \Travelport\Util\StructType\LegPrice[] $legPrice
     ...
     * Set Key value
     * @param string $key
     * @return \Travelport\Util\StructType\ExpertSolution
     */
    ...
    /**
     * This method is responsible for validating the value(s) passed to the setLegPrice method
     ...
     */
    public static function validateLegPriceForArrayConstraintFromSetLegPrice(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $expertSolutionLegPriceItem) {
            ...
            if (!$expertSolutionLegPriceItem instanceof \Travelport\Util\StructType\LegPrice) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The LegPrice property can only contain items of type \Travelport\Util\StructType\LegPrice, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    public function setLegPrice(?array $legPrice = null): self
    {
        ...
        if ('' !== ($legPriceArrayErrorMessage = self::validateLegPriceForArrayConstraintFromSetLegPrice($legPrice))) {
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\LegPrice $item
     * @return \Travelport\Util\StructType\ExpertSolution
     */
    public function addToLegPrice(\Travelport\Util\StructType\LegPrice $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\LegPrice) {
            throw new InvalidArgumentException(sprintf('The LegPrice property can only contain items of type \Travelport\Util\StructType\LegPrice, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
