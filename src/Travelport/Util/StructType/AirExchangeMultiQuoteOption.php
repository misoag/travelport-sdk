...
class AirExchangeMultiQuoteOption extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: AirSegmentData
     * @var \Travelport\Util\StructType\AirSegmentData[]
     */
     ...
     * @var \Travelport\Util\StructType\AirExchangeBundleList[]
     */
     ...
     * @param \Travelport\Util\StructType\AirSegmentData[] $airSegmentData
     * @param \Travelport\Util\StructType\AirExchangeBundleTotal $airExchangeBundleTotal
     * @param \Travelport\Util\StructType\AirExchangeBundleList[] $airExchangeBundleList
     */
    public function __construct(?array $airSegmentData = null, ?\Travelport\Util\StructType\AirExchangeBundleTotal $airExchangeBundleTotal = null, ?array $airExchangeBundleList = null)
    {
        $this
     ...
     * @return \Travelport\Util\StructType\AirSegmentData[]
     */
    ...
    public static function validateAirSegmentDataForArrayConstraintsFromSetAirSegmentData(?array $values = []): string
    {
        ...
        foreach ($values as $airExchangeMultiQuoteOptionAirSegmentDataItem) {
            ...
            if (!$airExchangeMultiQuoteOptionAirSegmentDataItem instanceof \Travelport\Util\StructType\AirSegmentData) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AirSegmentData property can only contain items of type \Travelport\Util\StructType\AirSegmentData, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\AirSegmentData $item
     * @return \Travelport\Util\StructType\AirExchangeMultiQuoteOption
     */
    public function addToAirSegmentData(\Travelport\Util\StructType\AirSegmentData $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\AirSegmentData) {
            throw new InvalidArgumentException(sprintf('The AirSegmentData property can only contain items of type \Travelport\Util\StructType\AirSegmentData, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateAirExchangeBundleListForArrayConstraintsFromSetAirExchangeBundleList(?array $values = []): string
    {
        ...
        foreach ($values as $airExchangeMultiQuoteOptionAirExchangeBundleListItem) {
            // validation for constraint: itemType
            if (!$airExchangeMultiQuoteOptionAirExchangeBundleListItem instanceof \Travelport\Util\StructType\AirExchangeBundleList) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AirExchangeBundleList property can only contain items of type \Travelport\Util\StructType\AirExchangeBundleList, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\AirExchangeBundleList $item
     * @return \Travelport\Util\StructType\AirExchangeMultiQuoteOption
     */
    public function addToAirExchangeBundleList(\Travelport\Util\StructType\AirExchangeBundleList $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\AirExchangeBundleList) {
            throw new InvalidArgumentException(sprintf('The AirExchangeBundleList property can only contain items of type \Travelport\Util\StructType\AirExchangeBundleList, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
