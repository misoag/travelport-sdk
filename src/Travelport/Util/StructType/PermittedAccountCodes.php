...
class PermittedAccountCodes extends AbstractStructBase
{
    ...
    public static function validateAccountCodeForArrayConstraintFromSetAccountCode(?array $values = []): string
    {
        ...
        $message = '';
        $invalidValues = [];
        foreach ($values as $permittedAccountCodesAccountCodeItem) {
            // validation for constraint: itemType
            if (!$permittedAccountCodesAccountCodeItem instanceof \Travelport\Util\StructType\AccountCode) {
                $invalidValues[] = is_object($permittedAccountCodesAccountCodeItem) ? get_class($permittedAccountCodesAccountCodeItem) : sprintf('%s(%s)', gettype($permittedAccountCodesAccountCodeItem), var_export($permittedAccountCodesAccountCodeItem, true));
            }
        }
     ...
     * @param \Travelport\Util\StructType\AccountCode[] $accountCode
     * @return \Travelport\Util\StructType\PermittedAccountCodes
     */
