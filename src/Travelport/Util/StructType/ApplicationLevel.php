...
class ApplicationLevel extends AbstractStructBase
{
     ...
     * Meta information extracted from the WSDL
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\ApplicationLimits|null
     */
    protected ?\Travelport\Util\StructType\ApplicationLimits $ApplicationLimits = null;
    /**
     * The ServiceData
     * Meta information extracted from the WSDL
     * - maxOccurs: 999
     ...
     * @param \Travelport\Util\StructType\ApplicationLimits $applicationLimits
     ...
     */
    public function __construct(?\Travelport\Util\StructType\ApplicationLimits $applicationLimits = null, ?array $serviceData = null, ?array $applicableLevels = null, ?string $providerDefinedApplicableLevels = null)
    {
        $this
     ...
     * @return \Travelport\Util\StructType\ApplicationLimits|null
     */
    public function getApplicationLimits(): ?\Travelport\Util\StructType\ApplicationLimits
    {
     ...
     * @param \Travelport\Util\StructType\ApplicationLimits $applicationLimits
     * @return \Travelport\Util\StructType\ApplicationLevel
     */
    public function setApplicationLimits(?\Travelport\Util\StructType\ApplicationLimits $applicationLimits = null): self
    {
    ...
    public static function validateServiceDataForArrayConstraintsFromSetServiceData(?array $values = []): string
    {
        ...
        foreach ($values as $applicationLevelServiceDataItem) {
            ...
            if (!$applicationLevelServiceDataItem instanceof \Travelport\Util\StructType\ServiceData) {
    ...
    public static function validateApplicableLevelsForArrayConstraintsFromSetApplicableLevels(?array $values = []): string
    {
        ...
        foreach ($values as $applicationLevelApplicableLevelsItem) {
            ...
            if (!\Travelport\Util\EnumType\OptionalServiceApplicabilityType::valueIsValid($applicationLevelApplicableLevelsItem)) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\OptionalServiceApplicabilityType', is_array($invalidValues) ? implode(', ', $invalidValues) : var_export($invalidValues, true), implode(', ', \Travelport\Util\EnumType\OptionalServiceApplicabilityType::getValidValues()));
        }
        unset($invalidValues);
     ...
     * @uses \Travelport\Util\EnumType\OptionalServiceApplicabilityType::valueIsValid()
     * @uses \Travelport\Util\EnumType\OptionalServiceApplicabilityType::getValidValues()
     * @throws InvalidArgumentException
     * @param string[] $applicableLevels
     * @return \Travelport\Util\StructType\ApplicationLevel
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\Util\EnumType\OptionalServiceApplicabilityType::valueIsValid($item)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\OptionalServiceApplicabilityType', is_array($item) ? implode(', ', $item) : var_export($item, true), implode(', ', \Travelport\Util\EnumType\OptionalServiceApplicabilityType::getValidValues())), __LINE__);
        }
