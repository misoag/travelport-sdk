...
class FareRuleShort extends AbstractStructBase
{
     ...
     * @var \Travelport\Util\StructType\FareRuleNameValue[]
     */
     ...
     * @param int $category
     * @param \Travelport\Util\StructType\FareRuleNameValue[] $fareRuleNameValue
     ...
     * @return \Travelport\Util\StructType\FareRuleNameValue[]
     */
    ...
    public static function validateFareRuleNameValueForArrayConstraintFromSetFareRuleNameValue(?array $values = []): string
    {
        ...
        foreach ($values as $fareRuleShortFareRuleNameValueItem) {
            ...
            if (!$fareRuleShortFareRuleNameValueItem instanceof \Travelport\Util\StructType\FareRuleNameValue) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The FareRuleNameValue property can only contain items of type \Travelport\Util\StructType\FareRuleNameValue, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\FareRuleNameValue $item
     * @return \Travelport\Util\StructType\FareRuleShort
     */
    public function addToFareRuleNameValue(\Travelport\Util\StructType\FareRuleNameValue $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\FareRuleNameValue) {
            throw new InvalidArgumentException(sprintf('The FareRuleNameValue property can only contain items of type \Travelport\Util\StructType\FareRuleNameValue, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
