...
class AvailableDiscount extends AbstractStructBase
{
     ...
     * @var \Travelport\Util\StructType\LoyaltyProgram[]
     */
     ...
     * @param \Travelport\Util\StructType\LoyaltyProgram[] $loyaltyProgram
     * @param string $amount
     ...
     * @return \Travelport\Util\StructType\LoyaltyProgram[]
     */
    ...
    public static function validateLoyaltyProgramForArrayConstraintsFromSetLoyaltyProgram(?array $values = []): string
    {
        ...
        foreach ($values as $availableDiscountLoyaltyProgramItem) {
            ...
            if (!$availableDiscountLoyaltyProgramItem instanceof \Travelport\Util\StructType\LoyaltyProgram) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The LoyaltyProgram property can only contain items of type \Travelport\Util\StructType\LoyaltyProgram, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\LoyaltyProgram $item
     * @return \Travelport\Util\StructType\AvailableDiscount
     */
    public function addToLoyaltyProgram(\Travelport\Util\StructType\LoyaltyProgram $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\LoyaltyProgram) {
            throw new InvalidArgumentException(sprintf('The LoyaltyProgram property can only contain items of type \Travelport\Util\StructType\LoyaltyProgram, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
