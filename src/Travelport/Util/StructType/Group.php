 ...
 * - documentation: Represents a traveler group for Group booking and all their accompanying data. SUPPORTED PROVIDER: Worldspan and JAL.
 * @subpackage Structs
 */
class Group extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: PhoneNumber
     * @var \Travelport\Util\StructType\PhoneNumber[]
     */
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\SSRRef[]
     */
     ...
     * @param \Travelport\Util\StructType\SSRRef[] $sSRRef
     ...
     */
    public function __construct(int $groupSize, ?\Travelport\Util\StructType\Name $name = null, ?\Travelport\Util\StructType\DeliveryInfo $deliveryInfo = null, ?array $phoneNumber = null, ?array $sSRRef = null, ?\Travelport\Util\StructType\TypeStructuredAddress $address = null, ?array $bookingTravelerRef = null, ?string $key = null, ?string $travelerType = null, ?string $elStat = null, ?bool $keyOverride = null)
    {
        $this
     ...
     * Set GroupSize value
     * @param int $groupSize
     * @return \Travelport\Util\StructType\Group
     */
     ...
     * @return \Travelport\Util\StructType\PhoneNumber[]
     */
    ...
    public static function validatePhoneNumberForArrayConstraintsFromSetPhoneNumber(?array $values = []): string
    {
        ...
        foreach ($values as $groupPhoneNumberItem) {
            ...
            if (!$groupPhoneNumberItem instanceof \Travelport\Util\StructType\PhoneNumber) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PhoneNumber property can only contain items of type \Travelport\Util\StructType\PhoneNumber, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\PhoneNumber $item
     * @return \Travelport\Util\StructType\Group
     */
    public function addToPhoneNumber(\Travelport\Util\StructType\PhoneNumber $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\PhoneNumber) {
            throw new InvalidArgumentException(sprintf('The PhoneNumber property can only contain items of type \Travelport\Util\StructType\PhoneNumber, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    /**
     * Get SSRRef value
     * @return \Travelport\Util\StructType\SSRRef[]
     */
    ...
    public static function validateSSRRefForArrayConstraintsFromSetSSRRef(?array $values = []): string
    {
        ...
        foreach ($values as $groupSSRRefItem) {
            // validation for constraint: itemType
            if (!$groupSSRRefItem instanceof \Travelport\Util\StructType\SSRRef) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SSRRef property can only contain items of type \Travelport\Util\StructType\SSRRef, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\SSRRef $item
     * @return \Travelport\Util\StructType\Group
     */
    public function addToSSRRef(\Travelport\Util\StructType\SSRRef $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\SSRRef) {
            throw new InvalidArgumentException(sprintf('The SSRRef property can only contain items of type \Travelport\Util\StructType\SSRRef, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateBookingTravelerRefForArrayConstraintsFromSetBookingTravelerRef(?array $values = []): string
    {
        ...
        foreach ($values as $groupBookingTravelerRefItem) {
            // validation for constraint: itemType
            if (!$groupBookingTravelerRefItem instanceof \Travelport\Util\StructType\BookingTravelerRef) {
