...
class PreferredSupplier extends AbstractStructBase
{
     ...
     * @uses \Travelport\Util\EnumType\TypeProfileType::valueIsValid()
     * @uses \Travelport\Util\EnumType\TypeProfileType::getValidValues()
    ...
    public function setProfileType(string $profileType): self
    {
        ...
        if (!\Travelport\Util\EnumType\TypeProfileType::valueIsValid($profileType)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeProfileType', is_array($profileType) ? implode(', ', $profileType) : var_export($profileType, true), implode(', ', \Travelport\Util\EnumType\TypeProfileType::getValidValues())), __LINE__);
        }
