...
class APIProvider extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: TransactionType
     * @var \Travelport\Util\StructType\TransactionType|null
     */
    protected ?\Travelport\Util\StructType\TransactionType $TransactionType = null;
    /**
     * The AvailablePseudoCityCode
     * Meta information extracted from the WSDL
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\AvailablePseudoCityCode[]
     */
     ...
     * @param \Travelport\Util\StructType\TransactionType $transactionType
     * @param \Travelport\Util\StructType\AvailablePseudoCityCode[] $availablePseudoCityCode
     * @param string $supplierCode
     * @param string $iATACode
     */
    public function __construct(string $providerCode, ?\Travelport\Util\StructType\TransactionType $transactionType = null, ?array $availablePseudoCityCode = null, ?string $supplierCode = null, ?string $iATACode = null)
    {
        $this
     ...
     * @param string $providerCode
     * @return \Travelport\Util\StructType\APIProvider
     */
     ...
     * @return \Travelport\Util\StructType\TransactionType|null
     */
    public function getTransactionType(): ?\Travelport\Util\StructType\TransactionType
    {
     ...
     * @return \Travelport\Util\StructType\APIProvider
     */
    public function setTransactionType(?\Travelport\Util\StructType\TransactionType $transactionType = null): self
    {
     ...
     * @return \Travelport\Util\StructType\AvailablePseudoCityCode[]
     */
    ...
    public static function validateAvailablePseudoCityCodeForArrayConstraintsFromSetAvailablePseudoCityCode(?array $values = []): string
    {
        ...
        foreach ($values as $aPIProviderAvailablePseudoCityCodeItem) {
            ...
            if (!$aPIProviderAvailablePseudoCityCodeItem instanceof \Travelport\Util\StructType\AvailablePseudoCityCode) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AvailablePseudoCityCode property can only contain items of type \Travelport\Util\StructType\AvailablePseudoCityCode, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\AvailablePseudoCityCode $item
     * @return \Travelport\Util\StructType\APIProvider
     */
    public function addToAvailablePseudoCityCode(\Travelport\Util\StructType\AvailablePseudoCityCode $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\AvailablePseudoCityCode) {
            throw new InvalidArgumentException(sprintf('The AvailablePseudoCityCode property can only contain items of type \Travelport\Util\StructType\AvailablePseudoCityCode, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
