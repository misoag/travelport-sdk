...
class ExchangePenaltyInfo extends AbstractStructBase
{
     ...
     * @var \Travelport\Util\StructType\PenaltyInformation[]
     */
     ...
     * @param \Travelport\Util\StructType\PenaltyInformation[] $penaltyInformation
     * @param string $pTC
    ...
    public static function validatePenaltyInformationForArrayConstraintsFromSetPenaltyInformation(?array $values = []): string
    {
        ...
        foreach ($values as $exchangePenaltyInfoPenaltyInformationItem) {
            ...
            if (!$exchangePenaltyInfoPenaltyInformationItem instanceof \Travelport\Util\StructType\PenaltyInformation) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PenaltyInformation property can only contain items of type \Travelport\Util\StructType\PenaltyInformation, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\PenaltyInformation $item
     * @return \Travelport\Util\StructType\ExchangePenaltyInfo
     */
    public function addToPenaltyInformation(\Travelport\Util\StructType\PenaltyInformation $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\PenaltyInformation) {
            throw new InvalidArgumentException(sprintf('The PenaltyInformation property can only contain items of type \Travelport\Util\StructType\PenaltyInformation, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
