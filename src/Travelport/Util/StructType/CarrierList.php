...
class CarrierList extends AbstractStructBase
{
     ...
     * - maxOccurs: 6
     * - ref: CarrierCode
     * @var \Travelport\Util\StructType\CarrierCode[]
     */
     ...
     * @param \Travelport\Util\StructType\CarrierCode[] $carrierCode
     */
     ...
     * @param bool $includeCarrier
     * @return \Travelport\Util\StructType\CarrierList
     */
     ...
     * @return \Travelport\Util\StructType\CarrierCode[]
     */
    ...
    public static function validateCarrierCodeForArrayConstraintsFromSetCarrierCode(?array $values = []): string
    {
        ...
        foreach ($values as $carrierListCarrierCodeItem) {
            ...
            if (!$carrierListCarrierCodeItem instanceof \Travelport\Util\StructType\CarrierCode) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The CarrierCode property can only contain items of type \Travelport\Util\StructType\CarrierCode, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\CarrierCode $item
     * @return \Travelport\Util\StructType\CarrierList
     */
    public function addToCarrierCode(\Travelport\Util\StructType\CarrierCode $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\CarrierCode) {
            throw new InvalidArgumentException(sprintf('The CarrierCode property can only contain items of type \Travelport\Util\StructType\CarrierCode, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(6)
