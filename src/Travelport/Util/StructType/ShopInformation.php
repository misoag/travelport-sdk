...
class ShopInformation extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\SearchRequest[]
     */
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\FlightsOffered[]
     */
     ...
     * @param \Travelport\Util\StructType\SearchRequest[] $searchRequest
     * @param \Travelport\Util\StructType\FlightsOffered[] $flightsOffered
     ...
     * @return \Travelport\Util\StructType\SearchRequest[]
     */
    ...
    public static function validateSearchRequestForArrayConstraintFromSetSearchRequest(?array $values = []): string
    {
        ...
        foreach ($values as $shopInformationSearchRequestItem) {
            ...
            if (!$shopInformationSearchRequestItem instanceof \Travelport\Util\StructType\SearchRequest) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SearchRequest property can only contain items of type \Travelport\Util\StructType\SearchRequest, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\SearchRequest[] $searchRequest
     * @return \Travelport\Util\StructType\ShopInformation
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\SearchRequest $item
     * @return \Travelport\Util\StructType\ShopInformation
     */
    public function addToSearchRequest(\Travelport\Util\StructType\SearchRequest $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\SearchRequest) {
            throw new InvalidArgumentException(sprintf('The SearchRequest property can only contain items of type \Travelport\Util\StructType\SearchRequest, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
     ...
     * @return \Travelport\Util\StructType\FlightsOffered[]
     */
    ...
    public static function validateFlightsOfferedForArrayConstraintFromSetFlightsOffered(?array $values = []): string
    {
        ...
        foreach ($values as $shopInformationFlightsOfferedItem) {
            // validation for constraint: itemType
            if (!$shopInformationFlightsOfferedItem instanceof \Travelport\Util\StructType\FlightsOffered) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The FlightsOffered property can only contain items of type \Travelport\Util\StructType\FlightsOffered, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\FlightsOffered $item
     * @return \Travelport\Util\StructType\ShopInformation
     */
    public function addToFlightsOffered(\Travelport\Util\StructType\FlightsOffered $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\FlightsOffered) {
            throw new InvalidArgumentException(sprintf('The FlightsOffered property can only contain items of type \Travelport\Util\StructType\FlightsOffered, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
