...
class Rows extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: Row
     * @var \Travelport\Util\StructType\Row[]
     */
     ...
     * @param \Travelport\Util\StructType\Row[] $row
    ...
    /**
     * Get Row value
     * @return \Travelport\Util\StructType\Row[]
     */
    ...
    /**
     * This method is responsible for validating the value(s) passed to the setRow method
     ...
     */
    public static function validateRowForArrayConstraintFromSetRow(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $rowsRowItem) {
            ...
            if (!$rowsRowItem instanceof \Travelport\Util\StructType\Row) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Row property can only contain items of type \Travelport\Util\StructType\Row, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\Row[] $row
     * @return \Travelport\Util\StructType\Rows
     */
    public function setRow(?array $row = null): self
    {
        // validation for constraint: array
        if ('' !== ($rowArrayErrorMessage = self::validateRowForArrayConstraintFromSetRow($row))) {
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\Row $item
     * @return \Travelport\Util\StructType\Rows
     */
    public function addToRow(\Travelport\Util\StructType\Row $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\Row) {
            throw new InvalidArgumentException(sprintf('The Row property can only contain items of type \Travelport\Util\StructType\Row, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
