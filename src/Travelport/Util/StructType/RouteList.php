...
class RouteList extends AbstractStructBase
{
    ...
    /**
     * Get Route value
     * @return \Travelport\Util\StructType\Route[]
     */
    ...
    public static function validateRouteForArrayConstraintsFromSetRoute(?array $values = []): string
    {
        ...
        foreach ($values as $routeListRouteItem) {
            ...
            if (!$routeListRouteItem instanceof \Travelport\Util\StructType\Route) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Route property can only contain items of type \Travelport\Util\StructType\Route, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\Route[] $route
     * @return \Travelport\Util\StructType\RouteList
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\Route $item
     * @return \Travelport\Util\StructType\RouteList
     */
    public function addToRoute(\Travelport\Util\StructType\Route $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\Route) {
            throw new InvalidArgumentException(sprintf('The Route property can only contain items of type \Travelport\Util\StructType\Route, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
