...
class QueuePlace extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: QueueSelector
     * @var \Travelport\Util\StructType\QueueSelector[]
     */
     ...
     * @param \Travelport\Util\StructType\QueueSelector[] $queueSelector
     */
     ...
     * @param string $pseudoCityCode
     * @return \Travelport\Util\StructType\QueuePlace
     */
     ...
     * @return \Travelport\Util\StructType\QueueSelector[]
     */
    ...
    public static function validateQueueSelectorForArrayConstraintsFromSetQueueSelector(?array $values = []): string
    {
        ...
        foreach ($values as $queuePlaceQueueSelectorItem) {
            ...
            if (!$queuePlaceQueueSelectorItem instanceof \Travelport\Util\StructType\QueueSelector) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The QueueSelector property can only contain items of type \Travelport\Util\StructType\QueueSelector, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\QueueSelector $item
     * @return \Travelport\Util\StructType\QueuePlace
     */
    public function addToQueueSelector(\Travelport\Util\StructType\QueueSelector $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\QueueSelector) {
            throw new InvalidArgumentException(sprintf('The QueueSelector property can only contain items of type \Travelport\Util\StructType\QueueSelector, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
