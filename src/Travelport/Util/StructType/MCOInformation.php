...
class MCOInformation extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: PassengerInfo
     * @var \Travelport\Util\StructType\PassengerInfo[]
     */
     ...
     * @param \Travelport\Util\StructType\PassengerInfo[] $passengerInfo
     ...
     * @return \Travelport\Util\StructType\PassengerInfo[]
     */
    ...
    public static function validatePassengerInfoForArrayConstraintsFromSetPassengerInfo(?array $values = []): string
    {
        ...
        foreach ($values as $mCOInformationPassengerInfoItem) {
            ...
            if (!$mCOInformationPassengerInfoItem instanceof \Travelport\Util\StructType\PassengerInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PassengerInfo property can only contain items of type \Travelport\Util\StructType\PassengerInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\PassengerInfo $item
     * @return \Travelport\Util\StructType\MCOInformation
     */
    public function addToPassengerInfo(\Travelport\Util\StructType\PassengerInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\PassengerInfo) {
            throw new InvalidArgumentException(sprintf('The PassengerInfo property can only contain items of type \Travelport\Util\StructType\PassengerInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
