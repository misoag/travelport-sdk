...
class ServiceInfo extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: MediaItem
     * @var \Travelport\Util\StructType\MediaItem[]
     */
     ...
     * @param \Travelport\Util\StructType\MediaItem[] $mediaItem
     */
     ...
     * @param string[] $description
     * @return \Travelport\Util\StructType\ServiceInfo
     */
    ...
    public static function validateMediaItemForArrayConstraintFromSetMediaItem(?array $values = []): string
    {
        ...
        foreach ($values as $serviceInfoMediaItemItem) {
            // validation for constraint: itemType
            if (!$serviceInfoMediaItemItem instanceof \Travelport\Util\StructType\MediaItem) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MediaItem property can only contain items of type \Travelport\Util\StructType\MediaItem, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\MediaItem $item
     * @return \Travelport\Util\StructType\ServiceInfo
     */
    public function addToMediaItem(\Travelport\Util\StructType\MediaItem $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\MediaItem) {
            throw new InvalidArgumentException(sprintf('The MediaItem property can only contain items of type \Travelport\Util\StructType\MediaItem, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(3)
