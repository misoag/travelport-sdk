...
class EMDSummaryInfo extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - ref: EMDSummary
     * @var \Travelport\Util\StructType\EMDSummary[]
     */
     ...
     * @param \Travelport\Util\StructType\EMDSummary[] $eMDSummary
     ...
     */
    public function __construct(?array $eMDSummary = null, ?\Travelport\Util\StructType\EMDTravelerInfo $eMDTravelerInfo = null, ?\Travelport\Util\StructType\Payment $payment = null, ?string $providerReservationInfoRef = null, ?string $key = null, ?string $elStat = null, ?bool $keyOverride = null)
    {
        $this
     ...
     * @return \Travelport\Util\StructType\EMDSummary[]
     */
    ...
    /**
     * This method is responsible for validating the value(s) passed to the setEMDSummary method
     ...
     */
    public static function validateEMDSummaryForArrayConstraintFromSetEMDSummary(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $eMDSummaryInfoEMDSummaryItem) {
            ...
            if (!$eMDSummaryInfoEMDSummaryItem instanceof \Travelport\Util\StructType\EMDSummary) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The EMDSummary property can only contain items of type \Travelport\Util\StructType\EMDSummary, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\EMDSummary[] $eMDSummary
     * @return \Travelport\Util\StructType\EMDSummaryInfo
     */
    public function setEMDSummary(?array $eMDSummary = null): self
    {
        // validation for constraint: array
        if ('' !== ($eMDSummaryArrayErrorMessage = self::validateEMDSummaryForArrayConstraintFromSetEMDSummary($eMDSummary))) {
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\EMDSummary $item
     * @return \Travelport\Util\StructType\EMDSummaryInfo
     */
    public function addToEMDSummary(\Travelport\Util\StructType\EMDSummary $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\EMDSummary) {
            throw new InvalidArgumentException(sprintf('The EMDSummary property can only contain items of type \Travelport\Util\StructType\EMDSummary, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
