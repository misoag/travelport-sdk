...
class FareGuaranteeInfo extends AbstractStructBase
{
     ...
     * @uses \Travelport\Util\EnumType\TypeFareGuarantee::valueIsValid()
     * @uses \Travelport\Util\EnumType\TypeFareGuarantee::getValidValues()
     * @throws InvalidArgumentException
     * @param string $guaranteeType
     * @return \Travelport\Util\StructType\FareGuaranteeInfo
     */
    public function setGuaranteeType(string $guaranteeType): self
    {
        // validation for constraint: enumeration
        if (!\Travelport\Util\EnumType\TypeFareGuarantee::valueIsValid($guaranteeType)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeFareGuarantee', is_array($guaranteeType) ? implode(', ', $guaranteeType) : var_export($guaranteeType, true), implode(', ', \Travelport\Util\EnumType\TypeFareGuarantee::getValidValues())), __LINE__);
        }
