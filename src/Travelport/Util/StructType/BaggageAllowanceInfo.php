...
class BaggageAllowanceInfo extends BaseBaggageAllowanceInfo
{
     ...
     * - minOccurs: 0
     * - ref: BagDetails
     * @var \Travelport\Util\StructType\BagDetails[]
     */
     ...
     * @param \Travelport\Util\StructType\BagDetails[] $bagDetails
    ...
    /**
     * This method is responsible for validating the values passed to the setBagDetails method
     ...
     */
    public static function validateBagDetailsForArrayConstraintsFromSetBagDetails(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $baggageAllowanceInfoBagDetailsItem) {
            ...
            if (!$baggageAllowanceInfoBagDetailsItem instanceof \Travelport\Util\StructType\BagDetails) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The BagDetails property can only contain items of type \Travelport\Util\StructType\BagDetails, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    public function setBagDetails(?array $bagDetails = null): self
    {
        ...
        if ('' !== ($bagDetailsArrayErrorMessage = self::validateBagDetailsForArrayConstraintsFromSetBagDetails($bagDetails))) {
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\BagDetails $item
     * @return \Travelport\Util\StructType\BaggageAllowanceInfo
     */
    public function addToBagDetails(\Travelport\Util\StructType\BagDetails $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\BagDetails) {
            throw new InvalidArgumentException(sprintf('The BagDetails property can only contain items of type \Travelport\Util\StructType\BagDetails, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
