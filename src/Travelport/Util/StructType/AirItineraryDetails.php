...
class AirItineraryDetails extends AbstractStructBase
{
     ...
     * @var \Travelport\Util\StructType\AirSegmentDetails[]
     */
     ...
     * @var \Travelport\Util\StructType\PassengerDetails[]
     */
     ...
     * @param \Travelport\Util\StructType\AirSegmentDetails[] $airSegmentDetails
     * @param string $key
     * @param \Travelport\Util\StructType\PassengerDetails[] $passengerDetails
     */
     ...
     * @return \Travelport\Util\StructType\AirSegmentDetails[]
     */
    ...
    public static function validateAirSegmentDetailsForArrayConstraintFromSetAirSegmentDetails(?array $values = []): string
    {
        ...
        foreach ($values as $airItineraryDetailsAirSegmentDetailsItem) {
            ...
            if (!$airItineraryDetailsAirSegmentDetailsItem instanceof \Travelport\Util\StructType\AirSegmentDetails) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AirSegmentDetails property can only contain items of type \Travelport\Util\StructType\AirSegmentDetails, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\AirSegmentDetails $item
     * @return \Travelport\Util\StructType\AirItineraryDetails
     */
    public function addToAirSegmentDetails(\Travelport\Util\StructType\AirSegmentDetails $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\AirSegmentDetails) {
            throw new InvalidArgumentException(sprintf('The AirSegmentDetails property can only contain items of type \Travelport\Util\StructType\AirSegmentDetails, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(16)
     ...
     * @return \Travelport\Util\StructType\PassengerDetails[]
     */
    ...
    public static function validatePassengerDetailsForArrayConstraintFromSetPassengerDetails(?array $values = []): string
    {
        ...
        foreach ($values as $airItineraryDetailsPassengerDetailsItem) {
            // validation for constraint: itemType
            if (!$airItineraryDetailsPassengerDetailsItem instanceof \Travelport\Util\StructType\PassengerDetails) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PassengerDetails property can only contain items of type \Travelport\Util\StructType\PassengerDetails, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\PassengerDetails $item
     * @return \Travelport\Util\StructType\AirItineraryDetails
     */
    public function addToPassengerDetails(\Travelport\Util\StructType\PassengerDetails $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\PassengerDetails) {
            throw new InvalidArgumentException(sprintf('The PassengerDetails property can only contain items of type \Travelport\Util\StructType\PassengerDetails, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(15)
