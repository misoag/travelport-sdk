 ...
 * - documentation: Fare Reference associated with the BookingRules | Which activities are supported for a particular element
 * @subpackage Structs
 */
class Restriction extends AbstractStructBase
{
     ...
     * - maxOccurs: 3
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\DaysOfWeekRestriction[]
     */
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\RestrictionPassengerTypes[]
     */
     ...
     * @param string $reason
     * @param \Travelport\Util\StructType\DaysOfWeekRestriction[] $daysOfWeekRestriction
     * @param \Travelport\Util\StructType\RestrictionPassengerTypes[] $restrictionPassengerTypes
     */
    ...
    public static function validateDaysOfWeekRestrictionForArrayConstraintFromSetDaysOfWeekRestriction(?array $values = []): string
    {
        ...
        foreach ($values as $restrictionDaysOfWeekRestrictionItem) {
            ...
            if (!$restrictionDaysOfWeekRestrictionItem instanceof \Travelport\Util\StructType\DaysOfWeekRestriction) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The DaysOfWeekRestriction property can only contain items of type \Travelport\Util\StructType\DaysOfWeekRestriction, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\DaysOfWeekRestriction $item
     * @return \Travelport\Util\StructType\Restriction
     */
    public function addToDaysOfWeekRestriction(\Travelport\Util\StructType\DaysOfWeekRestriction $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\DaysOfWeekRestriction) {
            throw new InvalidArgumentException(sprintf('The DaysOfWeekRestriction property can only contain items of type \Travelport\Util\StructType\DaysOfWeekRestriction, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(3)
     ...
     * @return \Travelport\Util\StructType\RestrictionPassengerTypes[]
     */
    ...
    public static function validateRestrictionPassengerTypesForArrayConstraintFromSetRestrictionPassengerTypes(?array $values = []): string
    {
        ...
        foreach ($values as $restrictionRestrictionPassengerTypesItem) {
            // validation for constraint: itemType
            if (!$restrictionRestrictionPassengerTypesItem instanceof \Travelport\Util\StructType\RestrictionPassengerTypes) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The RestrictionPassengerTypes property can only contain items of type \Travelport\Util\StructType\RestrictionPassengerTypes, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\RestrictionPassengerTypes $item
     * @return \Travelport\Util\StructType\Restriction
     */
    public function addToRestrictionPassengerTypes(\Travelport\Util\StructType\RestrictionPassengerTypes $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\RestrictionPassengerTypes) {
            throw new InvalidArgumentException(sprintf('The RestrictionPassengerTypes property can only contain items of type \Travelport\Util\StructType\RestrictionPassengerTypes, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
