...
class ConsolidatorRemark extends AbstractStructBase
{
     ...
     * - minOccurs: 1
     * - ref: PseudoCityCode
     * @var \Travelport\Util\StructType\PseudoCityCode[]
     */
     ...
     * @param \Travelport\Util\StructType\PseudoCityCode[] $pseudoCityCode
     * @param string $key
    ...
    public static function validatePseudoCityCodeForArrayConstraintFromSetPseudoCityCode(?array $values = []): string
    {
        ...
        foreach ($values as $consolidatorRemarkPseudoCityCodeItem) {
            ...
            if (!$consolidatorRemarkPseudoCityCodeItem instanceof \Travelport\Util\StructType\PseudoCityCode) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PseudoCityCode property can only contain items of type \Travelport\Util\StructType\PseudoCityCode, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\PseudoCityCode[] $pseudoCityCode
     * @return \Travelport\Util\StructType\ConsolidatorRemark
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\PseudoCityCode $item
     * @return \Travelport\Util\StructType\ConsolidatorRemark
     */
    public function addToPseudoCityCode(\Travelport\Util\StructType\PseudoCityCode $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\PseudoCityCode) {
            throw new InvalidArgumentException(sprintf('The PseudoCityCode property can only contain items of type \Travelport\Util\StructType\PseudoCityCode, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(5)
