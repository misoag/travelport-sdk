...
class SegmentSelect extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\TypeNonAirReservationRef[]
     */
    ...
    public static function validateAirSegmentRefForArrayConstraintFromSetAirSegmentRef(?array $values = []): string
    {
        ...
        foreach ($values as $segmentSelectAirSegmentRefItem) {
            ...
            if (!$segmentSelectAirSegmentRefItem instanceof \Travelport\Util\StructType\TypeSegmentRef) {
     ...
     * @param \Travelport\Util\StructType\TypeSegmentRef[] $airSegmentRef
     * @return \Travelport\Util\StructType\SegmentSelect
     */
     ...
     * @return \Travelport\Util\StructType\TypeNonAirReservationRef[]
     */
    ...
    public static function validateHotelReservationRefForArrayConstraintFromSetHotelReservationRef(?array $values = []): string
    {
        ...
        foreach ($values as $segmentSelectHotelReservationRefItem) {
            // validation for constraint: itemType
            if (!$segmentSelectHotelReservationRefItem instanceof \Travelport\Util\StructType\TypeNonAirReservationRef) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The HotelReservationRef property can only contain items of type \Travelport\Util\StructType\TypeNonAirReservationRef, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\TypeNonAirReservationRef $item
     * @return \Travelport\Util\StructType\SegmentSelect
     */
    public function addToHotelReservationRef(\Travelport\Util\StructType\TypeNonAirReservationRef $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\TypeNonAirReservationRef) {
            throw new InvalidArgumentException(sprintf('The HotelReservationRef property can only contain items of type \Travelport\Util\StructType\TypeNonAirReservationRef, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateVehicleReservationRefForArrayConstraintFromSetVehicleReservationRef(?array $values = []): string
    {
        ...
        foreach ($values as $segmentSelectVehicleReservationRefItem) {
            // validation for constraint: itemType
            if (!$segmentSelectVehicleReservationRefItem instanceof \Travelport\Util\StructType\TypeNonAirReservationRef) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The VehicleReservationRef property can only contain items of type \Travelport\Util\StructType\TypeNonAirReservationRef, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @return \Travelport\Util\StructType\SegmentSelect
     */
    public function addToVehicleReservationRef(\Travelport\Util\StructType\TypeNonAirReservationRef $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\TypeNonAirReservationRef) {
            throw new InvalidArgumentException(sprintf('The VehicleReservationRef property can only contain items of type \Travelport\Util\StructType\TypeNonAirReservationRef, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validatePassiveSegmentRefForArrayConstraintFromSetPassiveSegmentRef(?array $values = []): string
    {
        ...
        foreach ($values as $segmentSelectPassiveSegmentRefItem) {
            // validation for constraint: itemType
            if (!$segmentSelectPassiveSegmentRefItem instanceof \Travelport\Util\StructType\TypeSegmentRef) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PassiveSegmentRef property can only contain items of type \Travelport\Util\StructType\TypeSegmentRef, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @return \Travelport\Util\StructType\SegmentSelect
     */
    public function addToPassiveSegmentRef(\Travelport\Util\StructType\TypeSegmentRef $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\TypeSegmentRef) {
            throw new InvalidArgumentException(sprintf('The PassiveSegmentRef property can only contain items of type \Travelport\Util\StructType\TypeSegmentRef, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
