...
class LandCharges extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: Tax
     * @var \Travelport\Util\StructType\Tax[]
     */
     ...
     * @param \Travelport\Util\StructType\Tax[] $tax
     * @param string $base
     * @param string $total
    ...
    public static function validateTaxForArrayConstraintFromSetTax(?array $values = []): string
    {
        ...
        foreach ($values as $landChargesTaxItem) {
            ...
            if (!$landChargesTaxItem instanceof \Travelport\Util\StructType\Tax) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Tax property can only contain items of type \Travelport\Util\StructType\Tax, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\Tax[] $tax
     * @return \Travelport\Util\StructType\LandCharges
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\Tax $item
     * @return \Travelport\Util\StructType\LandCharges
     */
    public function addToTax(\Travelport\Util\StructType\Tax $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\Tax) {
            throw new InvalidArgumentException(sprintf('The Tax property can only contain items of type \Travelport\Util\StructType\Tax, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(3)
