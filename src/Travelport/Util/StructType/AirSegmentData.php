...
class AirSegmentData extends AbstractStructBase
{
     ...
     * @var \Travelport\Util\StructType\BaggageAllowance[]
     */
     ...
     * - minOccurs: 0
     * - ref: Brand
     * @var \Travelport\Util\StructType\Brand[]
     */
    ...
    public static function validateAirSegmentRefForArrayConstraintsFromSetAirSegmentRef(?array $values = []): string
    {
        ...
        foreach ($values as $airSegmentDataAirSegmentRefItem) {
            ...
            if (!$airSegmentDataAirSegmentRefItem instanceof \Travelport\Util\StructType\AirSegmentRef) {
    ...
    public static function validateBaggageAllowanceForArrayConstraintsFromSetBaggageAllowance(?array $values = []): string
    {
        ...
        foreach ($values as $airSegmentDataBaggageAllowanceItem) {
            // validation for constraint: itemType
            if (!$airSegmentDataBaggageAllowanceItem instanceof \Travelport\Util\StructType\BaggageAllowance) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The BaggageAllowance property can only contain items of type \Travelport\Util\StructType\BaggageAllowance, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\BaggageAllowance $item
     * @return \Travelport\Util\StructType\AirSegmentData
     */
    public function addToBaggageAllowance(\Travelport\Util\StructType\BaggageAllowance $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\BaggageAllowance) {
            throw new InvalidArgumentException(sprintf('The BaggageAllowance property can only contain items of type \Travelport\Util\StructType\BaggageAllowance, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateBrandForArrayConstraintsFromSetBrand(?array $values = []): string
    {
        ...
        foreach ($values as $airSegmentDataBrandItem) {
            // validation for constraint: itemType
            if (!$airSegmentDataBrandItem instanceof \Travelport\Util\StructType\Brand) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Brand property can only contain items of type \Travelport\Util\StructType\Brand, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\Brand $item
     * @return \Travelport\Util\StructType\AirSegmentData
     */
    public function addToBrand(\Travelport\Util\StructType\Brand $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\Brand) {
            throw new InvalidArgumentException(sprintf('The Brand property can only contain items of type \Travelport\Util\StructType\Brand, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
