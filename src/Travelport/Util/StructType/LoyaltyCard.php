...
class LoyaltyCard extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\TypeProviderReservationSpecificInfo[]
     */
     ...
     * @param \Travelport\Util\StructType\TypeProviderReservationSpecificInfo[] $providerReservationSpecificInfo
     * @param string $status
    ...
    public static function validateProviderReservationSpecificInfoForArrayConstraintFromSetProviderReservationSpecificInfo(?array $values = []): string
    {
        ...
        foreach ($values as $loyaltyCardProviderReservationSpecificInfoItem) {
            ...
            if (!$loyaltyCardProviderReservationSpecificInfoItem instanceof \Travelport\Util\StructType\TypeProviderReservationSpecificInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ProviderReservationSpecificInfo property can only contain items of type \Travelport\Util\StructType\TypeProviderReservationSpecificInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\TypeProviderReservationSpecificInfo $item
     * @return \Travelport\Util\StructType\LoyaltyCard
     */
    public function addToProviderReservationSpecificInfo(\Travelport\Util\StructType\TypeProviderReservationSpecificInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\TypeProviderReservationSpecificInfo) {
            throw new InvalidArgumentException(sprintf('The ProviderReservationSpecificInfo property can only contain items of type \Travelport\Util\StructType\TypeProviderReservationSpecificInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
     ...
     * @uses \Travelport\Util\EnumType\TypeProduct::valueIsValid()
     * @uses \Travelport\Util\EnumType\TypeProduct::getValidValues()
     * @throws InvalidArgumentException
     * @param string $supplierType
     * @return \Travelport\Util\StructType\LoyaltyCard
     */
    public function setSupplierType(?string $supplierType = null): self
    {
        ...
        if (!\Travelport\Util\EnumType\TypeProduct::valueIsValid($supplierType)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeProduct', is_array($supplierType) ? implode(', ', $supplierType) : var_export($supplierType, true), implode(', ', \Travelport\Util\EnumType\TypeProduct::getValidValues())), __LINE__);
        }
