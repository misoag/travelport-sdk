...
class AirPricePointList extends AbstractStructBase
{
    ...
    /**
     * This method is responsible for validating the value(s) passed to the setAirPricePoint method
     ...
     */
    public static function validateAirPricePointForArrayConstraintFromSetAirPricePoint(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $airPricePointListAirPricePointItem) {
            ...
            if (!$airPricePointListAirPricePointItem instanceof \Travelport\Util\StructType\AirPricePoint) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AirPricePoint property can only contain items of type \Travelport\Util\StructType\AirPricePoint, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\AirPricePoint[] $airPricePoint
     * @return \Travelport\Util\StructType\AirPricePointList
     */
    public function setAirPricePoint(?array $airPricePoint = null): self
    {
        // validation for constraint: array
        if ('' !== ($airPricePointArrayErrorMessage = self::validateAirPricePointForArrayConstraintFromSetAirPricePoint($airPricePoint))) {
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\AirPricePoint $item
     * @return \Travelport\Util\StructType\AirPricePointList
     */
    public function addToAirPricePoint(\Travelport\Util\StructType\AirPricePoint $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\AirPricePoint) {
            throw new InvalidArgumentException(sprintf('The AirPricePoint property can only contain items of type \Travelport\Util\StructType\AirPricePoint, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
