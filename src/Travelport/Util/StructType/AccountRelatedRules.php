...
class AccountRelatedRules extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: BookingRules
     * @var \Travelport\Util\StructType\BookingRules[]
     */
     ...
     * - minOccurs: 0
     * - ref: RoutingRules
     * @var \Travelport\Util\StructType\RoutingRules|null
     */
    protected ?\Travelport\Util\StructType\RoutingRules $RoutingRules = null;
    /**
     ...
     * @param \Travelport\Util\StructType\BookingRules[] $bookingRules
     * @param \Travelport\Util\StructType\RoutingRules $routingRules
     */
    public function __construct(?array $bookingRules = null, ?\Travelport\Util\StructType\RoutingRules $routingRules = null)
    {
        $this
    ...
    public static function validateBookingRulesForArrayConstraintsFromSetBookingRules(?array $values = []): string
    {
        ...
        foreach ($values as $accountRelatedRulesBookingRulesItem) {
            ...
            if (!$accountRelatedRulesBookingRulesItem instanceof \Travelport\Util\StructType\BookingRules) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The BookingRules property can only contain items of type \Travelport\Util\StructType\BookingRules, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\BookingRules[] $bookingRules
     * @return \Travelport\Util\StructType\AccountRelatedRules
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\BookingRules $item
     * @return \Travelport\Util\StructType\AccountRelatedRules
     */
    public function addToBookingRules(\Travelport\Util\StructType\BookingRules $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\BookingRules) {
            throw new InvalidArgumentException(sprintf('The BookingRules property can only contain items of type \Travelport\Util\StructType\BookingRules, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
     ...
     * @return \Travelport\Util\StructType\RoutingRules|null
     */
    public function getRoutingRules(): ?\Travelport\Util\StructType\RoutingRules
    {
     ...
     * @return \Travelport\Util\StructType\AccountRelatedRules
     */
    public function setRoutingRules(?\Travelport\Util\StructType\RoutingRules $routingRules = null): self
    {
