...
class Row extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: Facility
     * @var \Travelport\Util\StructType\Facility[]
     */
     ...
     * @param int $number
     * @param \Travelport\Util\StructType\Facility[] $facility
    ...
    public static function validateFacilityForArrayConstraintFromSetFacility(?array $values = []): string
    {
        ...
        foreach ($values as $rowFacilityItem) {
            ...
            if (!$rowFacilityItem instanceof \Travelport\Util\StructType\Facility) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Facility property can only contain items of type \Travelport\Util\StructType\Facility, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\Facility $item
     * @return \Travelport\Util\StructType\Row
     */
    public function addToFacility(\Travelport\Util\StructType\Facility $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\Facility) {
            throw new InvalidArgumentException(sprintf('The Facility property can only contain items of type \Travelport\Util\StructType\Facility, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateCharacteristicForArrayConstraintFromSetCharacteristic(?array $values = []): string
    {
        ...
        foreach ($values as $rowCharacteristicItem) {
            // validation for constraint: itemType
            if (!$rowCharacteristicItem instanceof \Travelport\Util\StructType\Characteristic) {
