...
class TypeOTAKeyword extends AbstractStructBase
{
     ...
     * - maxOccurs: 99
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\TypeOTASubKey[]
     */
     ...
     * @param string $name
     * @param \Travelport\Util\StructType\TypeOTASubKey[] $subKey
     * @param string[] $text
     * @param string $number
     ...
     * Set Name value
     * @param string $name
     * @return \Travelport\Util\StructType\TypeOTAKeyword
     */
    ...
    public static function validateSubKeyForArrayConstraintFromSetSubKey(?array $values = []): string
    {
        ...
        foreach ($values as $typeOTAKeywordSubKeyItem) {
            ...
            if (!$typeOTAKeywordSubKeyItem instanceof \Travelport\Util\StructType\TypeOTASubKey) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SubKey property can only contain items of type \Travelport\Util\StructType\TypeOTASubKey, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\TypeOTASubKey $item
     * @return \Travelport\Util\StructType\TypeOTAKeyword
     */
    public function addToSubKey(\Travelport\Util\StructType\TypeOTASubKey $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\TypeOTASubKey) {
            throw new InvalidArgumentException(sprintf('The SubKey property can only contain items of type \Travelport\Util\StructType\TypeOTASubKey, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
