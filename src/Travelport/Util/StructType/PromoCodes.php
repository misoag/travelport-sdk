...
class PromoCodes extends AbstractStructBase
{
     ...
     * - minOccurs: 1
     * - ref: PromoCode
     * @var \Travelport\Util\StructType\PromoCode[]
     */
    protected array $PromoCode;
    /**
     * Constructor method for PromoCodes
     * @uses PromoCodes::setPromoCode()
     * @param \Travelport\Util\StructType\PromoCode[] $promoCode
     */
    ...
    public static function validatePromoCodeForArrayConstraintFromSetPromoCode(?array $values = []): string
    {
        ...
        foreach ($values as $promoCodesPromoCodeItem) {
            ...
            if (!$promoCodesPromoCodeItem instanceof \Travelport\Util\StructType\PromoCode) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PromoCode property can only contain items of type \Travelport\Util\StructType\PromoCode, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\PromoCode[] $promoCode
     * @return \Travelport\Util\StructType\PromoCodes
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\PromoCode $item
     * @return \Travelport\Util\StructType\PromoCodes
     */
    public function addToPromoCode(\Travelport\Util\StructType\PromoCode $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\PromoCode) {
            throw new InvalidArgumentException(sprintf('The PromoCode property can only contain items of type \Travelport\Util\StructType\PromoCode, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
