...
class Segment extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: SegmentRemark
     * @var \Travelport\Util\StructType\SegmentRemark[]
     */
     ...
     * @param string $key
     * @param \Travelport\Util\StructType\SegmentRemark[] $segmentRemark
     * @param string $status
     * @param bool $passive
     ...
     * Set Key value
     * @param string $key
     * @return \Travelport\Util\StructType\Segment
     */
     ...
     * @return \Travelport\Util\StructType\SegmentRemark[]
     */
    ...
    public static function validateSegmentRemarkForArrayConstraintFromSetSegmentRemark(?array $values = []): string
    {
        ...
        foreach ($values as $segmentSegmentRemarkItem) {
            ...
            if (!$segmentSegmentRemarkItem instanceof \Travelport\Util\StructType\SegmentRemark) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SegmentRemark property can only contain items of type \Travelport\Util\StructType\SegmentRemark, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\SegmentRemark $item
     * @return \Travelport\Util\StructType\Segment
     */
    public function addToSegmentRemark(\Travelport\Util\StructType\SegmentRemark $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\SegmentRemark) {
            throw new InvalidArgumentException(sprintf('The SegmentRemark property can only contain items of type \Travelport\Util\StructType\SegmentRemark, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
