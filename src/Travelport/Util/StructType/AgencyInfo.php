...
class AgencyInfo extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - ref: AgentAction
     * @var \Travelport\Util\StructType\AgentAction[]
     */
    protected ?array $AgentAction = null;
    /**
     * Constructor method for AgencyInfo
     * @uses AgencyInfo::setAgentAction()
     * @param \Travelport\Util\StructType\AgentAction[] $agentAction
     */
     ...
     * @return \Travelport\Util\StructType\AgentAction[]
     */
    ...
    /**
     * This method is responsible for validating the value(s) passed to the setAgentAction method
     ...
     */
    public static function validateAgentActionForArrayConstraintFromSetAgentAction(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $agencyInfoAgentActionItem) {
            ...
            if (!$agencyInfoAgentActionItem instanceof \Travelport\Util\StructType\AgentAction) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AgentAction property can only contain items of type \Travelport\Util\StructType\AgentAction, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\AgentAction[] $agentAction
     * @return \Travelport\Util\StructType\AgencyInfo
     */
    public function setAgentAction(?array $agentAction = null): self
    {
        // validation for constraint: array
        if ('' !== ($agentActionArrayErrorMessage = self::validateAgentActionForArrayConstraintFromSetAgentAction($agentAction))) {
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\AgentAction $item
     * @return \Travelport\Util\StructType\AgencyInfo
     */
    public function addToAgentAction(\Travelport\Util\StructType\AgentAction $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\AgentAction) {
            throw new InvalidArgumentException(sprintf('The AgentAction property can only contain items of type \Travelport\Util\StructType\AgentAction, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
