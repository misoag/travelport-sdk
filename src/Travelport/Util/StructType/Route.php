...
class Route extends AbstractStructBase
{
    ...
    public static function validateLegForArrayConstraintsFromSetLeg(?array $values = []): string
    {
        ...
        $message = '';
        $invalidValues = [];
        foreach ($values as $routeLegItem) {
            // validation for constraint: itemType
            if (!$routeLegItem instanceof \Travelport\Util\StructType\Leg) {
                $invalidValues[] = is_object($routeLegItem) ? get_class($routeLegItem) : sprintf('%s(%s)', gettype($routeLegItem), var_export($routeLegItem, true));
            }
        }
