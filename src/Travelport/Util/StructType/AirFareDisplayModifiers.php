...
class AirFareDisplayModifiers extends AbstractStructBase
{
    ...
    public static function validateTripTypeForArrayConstraintFromSetTripType(?array $values = []): string
    {
        ...
        foreach ($values as $airFareDisplayModifiersTripTypeItem) {
            ...
            if (!\Travelport\Util\EnumType\TypeFareTripType::valueIsValid($airFareDisplayModifiersTripTypeItem)) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeFareTripType', is_array($invalidValues) ? implode(', ', $invalidValues) : var_export($invalidValues, true), implode(', ', \Travelport\Util\EnumType\TypeFareTripType::getValidValues()));
        }
        unset($invalidValues);
     ...
     * @param string[] $tripType
     * @return \Travelport\Util\StructType\AirFareDisplayModifiers
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\Util\EnumType\TypeFareTripType::valueIsValid($item)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeFareTripType', is_array($item) ? implode(', ', $item) : var_export($item, true), implode(', ', \Travelport\Util\EnumType\TypeFareTripType::getValidValues())), __LINE__);
        }
        // validation for constraint: maxOccurs(3)
    ...
    public static function validateFareSearchOptionForArrayConstraintFromSetFareSearchOption(?array $values = []): string
    {
        ...
        foreach ($values as $airFareDisplayModifiersFareSearchOptionItem) {
            // validation for constraint: enumeration
            if (!\Travelport\Util\EnumType\TypeFareSearchOption::valueIsValid($airFareDisplayModifiersFareSearchOptionItem)) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeFareSearchOption', is_array($invalidValues) ? implode(', ', $invalidValues) : var_export($invalidValues, true), implode(', ', \Travelport\Util\EnumType\TypeFareSearchOption::getValidValues()));
        }
        unset($invalidValues);
     ...
     * @uses \Travelport\Util\EnumType\TypeFareSearchOption::valueIsValid()
     * @uses \Travelport\Util\EnumType\TypeFareSearchOption::getValidValues()
     * @throws InvalidArgumentException
     * @param string[] $fareSearchOption
     * @return \Travelport\Util\StructType\AirFareDisplayModifiers
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\Util\EnumType\TypeFareSearchOption::valueIsValid($item)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeFareSearchOption', is_array($item) ? implode(', ', $item) : var_export($item, true), implode(', ', \Travelport\Util\EnumType\TypeFareSearchOption::getValidValues())), __LINE__);
        }
        // validation for constraint: maxOccurs(5)
