...
class Affiliations extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: TravelArranger
     * @var \Travelport\Util\StructType\TravelArranger[]
     */
    protected ?array $TravelArranger = null;
    /**
     * Constructor method for Affiliations
     * @uses Affiliations::setTravelArranger()
     * @param \Travelport\Util\StructType\TravelArranger[] $travelArranger
     */
     ...
     */
    public static function validateTravelArrangerForArrayConstraintsFromSetTravelArranger(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $affiliationsTravelArrangerItem) {
            ...
            if (!$affiliationsTravelArrangerItem instanceof \Travelport\Util\StructType\TravelArranger) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The TravelArranger property can only contain items of type \Travelport\Util\StructType\TravelArranger, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\TravelArranger[] $travelArranger
     * @return \Travelport\Util\StructType\Affiliations
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\TravelArranger $item
     * @return \Travelport\Util\StructType\Affiliations
     */
    public function addToTravelArranger(\Travelport\Util\StructType\TravelArranger $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\TravelArranger) {
            throw new InvalidArgumentException(sprintf('The TravelArranger property can only contain items of type \Travelport\Util\StructType\TravelArranger, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
