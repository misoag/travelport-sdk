...
class Auxdata extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * @var \Travelport\Util\StructType\Entry[]
     */
    protected ?array $Entry = null;
    /**
     * Constructor method for Auxdata
     * @uses Auxdata::setEntry()
     * @param \Travelport\Util\StructType\Entry[] $entry
     */
    ...
    /**
     * Get Entry value
     * @return \Travelport\Util\StructType\Entry[]
     */
    ...
    public static function validateEntryForArrayConstraintsFromSetEntry(?array $values = []): string
    {
        ...
        foreach ($values as $auxdataEntryItem) {
            ...
            if (!$auxdataEntryItem instanceof \Travelport\Util\StructType\Entry) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Entry property can only contain items of type \Travelport\Util\StructType\Entry, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\Entry[] $entry
     * @return \Travelport\Util\StructType\Auxdata
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\Entry $item
     * @return \Travelport\Util\StructType\Auxdata
     */
    public function addToEntry(\Travelport\Util\StructType\Entry $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\Entry) {
            throw new InvalidArgumentException(sprintf('The Entry property can only contain items of type \Travelport\Util\StructType\Entry, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
