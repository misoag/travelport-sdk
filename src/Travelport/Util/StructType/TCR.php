...
class TCR extends AbstractStructBase
{
     ...
     * @var \Travelport\Util\StructType\PassengerTicketNumber[]
     */
     ...
     * @var \Travelport\Util\StructType\RefundRemark[]
     */
     ...
     * @param \Travelport\Util\StructType\PassengerTicketNumber[] $passengerTicketNumber
     ...
     * @param \Travelport\Util\StructType\RefundRemark[] $refundRemark
     ...
     */
    public function __construct(string $tCRNumber, string $status, string $modifiedDate, string $basePrice, string $taxes, string $fees, bool $refundable, bool $exchangeable, bool $voidable, bool $modifiable, ?array $formOfPayment = null, ?array $payment = null, ?array $bookingTraveler = null, ?array $passengerTicketNumber = null, ?array $airPricingInfo = null, ?\Travelport\Util\StructType\AgencyInfo $agencyInfo = null, ?\Travelport\Util\StructType\AirReservationLocatorCode $airReservationLocatorCode = null, ?array $supplierLocator = null, ?array $refundRemark = null, ?string $confirmedDate = null, ?string $refundAccessCode = null, ?string $refundAmount = null, ?string $refundFee = null, ?string $forfeitAmount = null, ?string $providerCode = null, ?string $providerLocatorCode = null, ?string $supplierCode = null)
    {
        $this
     ...
     * Set TCRNumber value
     * @param string $tCRNumber
     * @return \Travelport\Util\StructType\TCR
     */
    ...
    /**
     * Set Status value
     * @uses \Travelport\Util\EnumType\TypeTCRStatus::valueIsValid()
     * @uses \Travelport\Util\EnumType\TypeTCRStatus::getValidValues()
    ...
    public function setStatus(string $status): self
    {
        ...
        if (!\Travelport\Util\EnumType\TypeTCRStatus::valueIsValid($status)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeTCRStatus', is_array($status) ? implode(', ', $status) : var_export($status, true), implode(', ', \Travelport\Util\EnumType\TypeTCRStatus::getValidValues())), __LINE__);
        }
        $this->Status = $status;
    ...
    public static function validateFormOfPaymentForArrayConstraintFromSetFormOfPayment(?array $values = []): string
    {
        ...
        foreach ($values as $tCRFormOfPaymentItem) {
            ...
            if (!$tCRFormOfPaymentItem instanceof \Travelport\Util\StructType\FormOfPayment) {
    ...
    public static function validatePaymentForArrayConstraintFromSetPayment(?array $values = []): string
    {
        ...
        foreach ($values as $tCRPaymentItem) {
            // validation for constraint: itemType
            if (!$tCRPaymentItem instanceof \Travelport\Util\StructType\Payment) {
    ...
    public static function validateBookingTravelerForArrayConstraintFromSetBookingTraveler(?array $values = []): string
    {
        ...
        foreach ($values as $tCRBookingTravelerItem) {
            // validation for constraint: itemType
            if (!$tCRBookingTravelerItem instanceof \Travelport\Util\StructType\BookingTraveler) {
     ...
     * @return \Travelport\Util\StructType\PassengerTicketNumber[]
     */
    ...
    public static function validatePassengerTicketNumberForArrayConstraintFromSetPassengerTicketNumber(?array $values = []): string
    {
        ...
        foreach ($values as $tCRPassengerTicketNumberItem) {
            // validation for constraint: itemType
            if (!$tCRPassengerTicketNumberItem instanceof \Travelport\Util\StructType\PassengerTicketNumber) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PassengerTicketNumber property can only contain items of type \Travelport\Util\StructType\PassengerTicketNumber, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\PassengerTicketNumber $item
     * @return \Travelport\Util\StructType\TCR
     */
    public function addToPassengerTicketNumber(\Travelport\Util\StructType\PassengerTicketNumber $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\PassengerTicketNumber) {
            throw new InvalidArgumentException(sprintf('The PassengerTicketNumber property can only contain items of type \Travelport\Util\StructType\PassengerTicketNumber, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateAirPricingInfoForArrayConstraintFromSetAirPricingInfo(?array $values = []): string
    {
        ...
        foreach ($values as $tCRAirPricingInfoItem) {
            // validation for constraint: itemType
            if (!$tCRAirPricingInfoItem instanceof \Travelport\Util\StructType\AirPricingInfo) {
    ...
    public static function validateSupplierLocatorForArrayConstraintFromSetSupplierLocator(?array $values = []): string
    {
        ...
        foreach ($values as $tCRSupplierLocatorItem) {
            // validation for constraint: itemType
            if (!$tCRSupplierLocatorItem instanceof \Travelport\Util\StructType\SupplierLocator) {
     ...
     * @return \Travelport\Util\StructType\RefundRemark[]
     */
    ...
    public static function validateRefundRemarkForArrayConstraintFromSetRefundRemark(?array $values = []): string
    {
        ...
        foreach ($values as $tCRRefundRemarkItem) {
            // validation for constraint: itemType
            if (!$tCRRefundRemarkItem instanceof \Travelport\Util\StructType\RefundRemark) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The RefundRemark property can only contain items of type \Travelport\Util\StructType\RefundRemark, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\RefundRemark $item
     * @return \Travelport\Util\StructType\TCR
     */
    public function addToRefundRemark(\Travelport\Util\StructType\RefundRemark $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\RefundRemark) {
            throw new InvalidArgumentException(sprintf('The RefundRemark property can only contain items of type \Travelport\Util\StructType\RefundRemark, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
