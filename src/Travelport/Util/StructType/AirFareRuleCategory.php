...
class AirFareRuleCategory extends AbstractStructBase
{
    ...
    public static function validateCategoryCodeForArrayConstraintsFromSetCategoryCode(?array $values = []): string
    {
        ...
        foreach ($values as $airFareRuleCategoryCategoryCodeItem) {
            ...
            if (!\Travelport\Util\EnumType\TypeFareRuleCategoryCode::valueIsValid($airFareRuleCategoryCategoryCodeItem)) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeFareRuleCategoryCode', is_array($invalidValues) ? implode(', ', $invalidValues) : var_export($invalidValues, true), implode(', ', \Travelport\Util\EnumType\TypeFareRuleCategoryCode::getValidValues()));
        }
        unset($invalidValues);
     ...
     * @uses \Travelport\Util\EnumType\TypeFareRuleCategoryCode::valueIsValid()
     * @uses \Travelport\Util\EnumType\TypeFareRuleCategoryCode::getValidValues()
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\Util\EnumType\TypeFareRuleCategoryCode::valueIsValid($item)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeFareRuleCategoryCode', is_array($item) ? implode(', ', $item) : var_export($item, true), implode(', ', \Travelport\Util\EnumType\TypeFareRuleCategoryCode::getValidValues())), __LINE__);
        }
        // validation for constraint: maxOccurs(10)
