...
class AlternateRouteList extends AbstractStructBase
{
    ...
    public static function validateAlternateRouteForArrayConstraintsFromSetAlternateRoute(?array $values = []): string
    {
        ...
        foreach ($values as $alternateRouteListAlternateRouteItem) {
            ...
            if (!$alternateRouteListAlternateRouteItem instanceof \Travelport\Util\StructType\AlternateRoute) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The AlternateRoute property can only contain items of type \Travelport\Util\StructType\AlternateRoute, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\AlternateRoute[] $alternateRoute
     * @return \Travelport\Util\StructType\AlternateRouteList
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\AlternateRoute $item
     * @return \Travelport\Util\StructType\AlternateRouteList
     */
    public function addToAlternateRoute(\Travelport\Util\StructType\AlternateRoute $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\AlternateRoute) {
            throw new InvalidArgumentException(sprintf('The AlternateRoute property can only contain items of type \Travelport\Util\StructType\AlternateRoute, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
