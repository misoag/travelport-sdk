...
class Itinerary extends AbstractStructBase
{
    ...
    /**
     * Set Type value
     * @uses \Travelport\Util\EnumType\TypeItinerary::valueIsValid()
     * @uses \Travelport\Util\EnumType\TypeItinerary::getValidValues()
     * @throws InvalidArgumentException
     * @param string $type
     * @return \Travelport\Util\StructType\Itinerary
     */
    public function setType(?string $type = null): self
    {
        // validation for constraint: enumeration
        if (!\Travelport\Util\EnumType\TypeItinerary::valueIsValid($type)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeItinerary', is_array($type) ? implode(', ', $type) : var_export($type, true), implode(', ', \Travelport\Util\EnumType\TypeItinerary::getValidValues())), __LINE__);
        }
        $this->Type = $type;
    ...
    /**
     * Set Option value
     * @uses \Travelport\Util\EnumType\TypeItineraryOption::valueIsValid()
     * @uses \Travelport\Util\EnumType\TypeItineraryOption::getValidValues()
     * @throws InvalidArgumentException
     * @param string $option
     * @return \Travelport\Util\StructType\Itinerary
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\Util\EnumType\TypeItineraryOption::valueIsValid($option)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeItineraryOption', is_array($option) ? implode(', ', $option) : var_export($option, true), implode(', ', \Travelport\Util\EnumType\TypeItineraryOption::getValidValues())), __LINE__);
        }
        $this->Option = $option;
