...
class RequestKeyMappings extends AbstractStructBase
{
     ...
     * @return \Travelport\Util\StructType\KeyMapping[]
     */
    ...
    public static function validateKeyMappingForArrayConstraintFromSetKeyMapping(?array $values = []): string
    {
        ...
        foreach ($values as $requestKeyMappingsKeyMappingItem) {
            ...
            if (!$requestKeyMappingsKeyMappingItem instanceof \Travelport\Util\StructType\KeyMapping) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The KeyMapping property can only contain items of type \Travelport\Util\StructType\KeyMapping, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\KeyMapping[] $keyMapping
     * @return \Travelport\Util\StructType\RequestKeyMappings
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\KeyMapping $item
     * @return \Travelport\Util\StructType\RequestKeyMappings
     */
    public function addToKeyMapping(\Travelport\Util\StructType\KeyMapping $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\KeyMapping) {
            throw new InvalidArgumentException(sprintf('The KeyMapping property can only contain items of type \Travelport\Util\StructType\KeyMapping, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
