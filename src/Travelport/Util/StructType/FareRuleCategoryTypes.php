...
class FareRuleCategoryTypes extends AbstractStructBase
{
     ...
     * - maxOccurs: 99
     * - minOccurs: 0
     * @var \Travelport\Util\StructType\VariableCategoryType[]
     */
     ...
     * @param \Travelport\Util\StructType\VariableCategoryType[] $variableCategoryDetails
     */
    ...
    public static function validateCategoryDetailsForArrayConstraintsFromSetCategoryDetails(?array $values = []): string
    {
        ...
        foreach ($values as $fareRuleCategoryTypesCategoryDetailsItem) {
            ...
            if (!$fareRuleCategoryTypesCategoryDetailsItem instanceof \Travelport\Util\StructType\ValueDetails) {
    ...
    public static function validateVariableCategoryDetailsForArrayConstraintsFromSetVariableCategoryDetails(?array $values = []): string
    {
        ...
        foreach ($values as $fareRuleCategoryTypesVariableCategoryDetailsItem) {
            // validation for constraint: itemType
            if (!$fareRuleCategoryTypesVariableCategoryDetailsItem instanceof \Travelport\Util\StructType\VariableCategoryType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The VariableCategoryDetails property can only contain items of type \Travelport\Util\StructType\VariableCategoryType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\VariableCategoryType $item
     * @return \Travelport\Util\StructType\FareRuleCategoryTypes
     */
    public function addToVariableCategoryDetails(\Travelport\Util\StructType\VariableCategoryType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\VariableCategoryType) {
            throw new InvalidArgumentException(sprintf('The VariableCategoryDetails property can only contain items of type \Travelport\Util\StructType\VariableCategoryType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
