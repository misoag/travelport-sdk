...
class Facility extends AbstractStructBase
{
     ...
     * @var \Travelport\Util\StructType\PassengerSeatPrice[]
     */
     ...
     * @param \Travelport\Util\StructType\PassengerSeatPrice[] $passengerSeatPrice
    ...
    /**
     * Set Type value
     * @uses \Travelport\Util\EnumType\TypeFacility::valueIsValid()
     * @uses \Travelport\Util\EnumType\TypeFacility::getValidValues()
     * @throws InvalidArgumentException
     * @param string $type
     * @return \Travelport\Util\StructType\Facility
     */
    public function setType(string $type): self
    {
        // validation for constraint: enumeration
        if (!\Travelport\Util\EnumType\TypeFacility::valueIsValid($type)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeFacility', is_array($type) ? implode(', ', $type) : var_export($type, true), implode(', ', \Travelport\Util\EnumType\TypeFacility::getValidValues())), __LINE__);
        }
        $this->Type = $type;
    ...
    public static function validateCharacteristicForArrayConstraintFromSetCharacteristic(?array $values = []): string
    {
        ...
        foreach ($values as $facilityCharacteristicItem) {
            ...
            if (!$facilityCharacteristicItem instanceof \Travelport\Util\StructType\Characteristic) {
    ...
    public static function validateRemarkForArrayConstraintFromSetRemark(?array $values = []): string
    {
        ...
        foreach ($values as $facilityRemarkItem) {
            // validation for constraint: itemType
            if (!$facilityRemarkItem instanceof \Travelport\Util\StructType\Remark) {
    ...
    /**
     * This method is responsible for validating the value(s) passed to the setPassengerSeatPrice method
     ...
     * @return string A non-empty message if the values does not match the validation rules
     */
    public static function validatePassengerSeatPriceForArrayConstraintFromSetPassengerSeatPrice(?array $values = []): string
    {
        if (!is_array($values)) {
            return '';
        }
        ...
        foreach ($values as $facilityPassengerSeatPriceItem) {
            // validation for constraint: itemType
            if (!$facilityPassengerSeatPriceItem instanceof \Travelport\Util\StructType\PassengerSeatPrice) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PassengerSeatPrice property can only contain items of type \Travelport\Util\StructType\PassengerSeatPrice, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: array
        if ('' !== ($passengerSeatPriceArrayErrorMessage = self::validatePassengerSeatPriceForArrayConstraintFromSetPassengerSeatPrice($passengerSeatPrice))) {
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\PassengerSeatPrice $item
     * @return \Travelport\Util\StructType\Facility
     */
    public function addToPassengerSeatPrice(\Travelport\Util\StructType\PassengerSeatPrice $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\PassengerSeatPrice) {
            throw new InvalidArgumentException(sprintf('The PassengerSeatPrice property can only contain items of type \Travelport\Util\StructType\PassengerSeatPrice, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateTaxInfoForArrayConstraintFromSetTaxInfo(?array $values = []): string
    {
        ...
        foreach ($values as $facilityTaxInfoItem) {
            // validation for constraint: itemType
            if (!$facilityTaxInfoItem instanceof \Travelport\Util\StructType\TypeTaxInfo) {
    ...
    public static function validateServiceDataForArrayConstraintFromSetServiceData(?array $values = []): string
    {
        ...
        foreach ($values as $facilityServiceDataItem) {
            // validation for constraint: itemType
            if (!$facilityServiceDataItem instanceof \Travelport\Util\StructType\ServiceData) {
     ...
     * @uses \Travelport\Util\EnumType\TypeSeatAvailability::valueIsValid()
     * @uses \Travelport\Util\EnumType\TypeSeatAvailability::getValidValues()
     * @throws InvalidArgumentException
     * @param string $availability
     * @return \Travelport\Util\StructType\Facility
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\Travelport\Util\EnumType\TypeSeatAvailability::valueIsValid($availability)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \Travelport\Util\EnumType\TypeSeatAvailability', is_array($availability) ? implode(', ', $availability) : var_export($availability, true), implode(', ', \Travelport\Util\EnumType\TypeSeatAvailability::getValidValues())), __LINE__);
        }
