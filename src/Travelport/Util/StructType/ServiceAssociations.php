...
class ServiceAssociations extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * @var \Travelport\Util\StructType\ApplicableSegment[]
     */
     ...
     * @param \Travelport\Util\StructType\ApplicableSegment[] $applicableSegment
     */
     ...
     * @return \Travelport\Util\StructType\ApplicableSegment[]
     */
    ...
    public static function validateApplicableSegmentForArrayConstraintFromSetApplicableSegment(?array $values = []): string
    {
        ...
        foreach ($values as $serviceAssociationsApplicableSegmentItem) {
            ...
            if (!$serviceAssociationsApplicableSegmentItem instanceof \Travelport\Util\StructType\ApplicableSegment) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ApplicableSegment property can only contain items of type \Travelport\Util\StructType\ApplicableSegment, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\ApplicableSegment[] $applicableSegment
     * @return \Travelport\Util\StructType\ServiceAssociations
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\ApplicableSegment $item
     * @return \Travelport\Util\StructType\ServiceAssociations
     */
    public function addToApplicableSegment(\Travelport\Util\StructType\ApplicableSegment $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\ApplicableSegment) {
            throw new InvalidArgumentException(sprintf('The ApplicableSegment property can only contain items of type \Travelport\Util\StructType\ApplicableSegment, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
