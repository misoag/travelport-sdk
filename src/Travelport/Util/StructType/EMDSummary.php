...
class EMDSummary extends AbstractStructBase
{
    ...
    public static function validateEMDCouponForArrayConstraintsFromSetEMDCoupon(?array $values = []): string
    {
        ...
        $message = '';
        $invalidValues = [];
        foreach ($values as $eMDSummaryEMDCouponItem) {
            // validation for constraint: itemType
            if (!$eMDSummaryEMDCouponItem instanceof \Travelport\Util\StructType\EMDCoupon) {
                $invalidValues[] = is_object($eMDSummaryEMDCouponItem) ? get_class($eMDSummaryEMDCouponItem) : sprintf('%s(%s)', gettype($eMDSummaryEMDCouponItem), var_export($eMDSummaryEMDCouponItem, true));
            }
        }
