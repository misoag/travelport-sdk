...
class PassengerDetails extends AbstractStructBase
{
     ...
     * @var \Travelport\Util\StructType\LoyaltyCardDetails[]
     */
     ...
     * @param string $key
     * @param string $code
     * @param \Travelport\Util\StructType\LoyaltyCardDetails[] $loyaltyCardDetails
     * @param int $age
     */
    ...
    public static function validateLoyaltyCardDetailsForArrayConstraintFromSetLoyaltyCardDetails(?array $values = []): string
    {
        ...
        foreach ($values as $passengerDetailsLoyaltyCardDetailsItem) {
            ...
            if (!$passengerDetailsLoyaltyCardDetailsItem instanceof \Travelport\Util\StructType\LoyaltyCardDetails) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The LoyaltyCardDetails property can only contain items of type \Travelport\Util\StructType\LoyaltyCardDetails, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\LoyaltyCardDetails $item
     * @return \Travelport\Util\StructType\PassengerDetails
     */
    public function addToLoyaltyCardDetails(\Travelport\Util\StructType\LoyaltyCardDetails $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\LoyaltyCardDetails) {
            throw new InvalidArgumentException(sprintf('The LoyaltyCardDetails property can only contain items of type \Travelport\Util\StructType\LoyaltyCardDetails, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(9)
