...
class ExpertSolutionList extends AbstractStructBase
{
    ...
    public static function validateExpertSolutionForArrayConstraintFromSetExpertSolution(?array $values = []): string
    {
        ...
        foreach ($values as $expertSolutionListExpertSolutionItem) {
            ...
            if (!$expertSolutionListExpertSolutionItem instanceof \Travelport\Util\StructType\ExpertSolution) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ExpertSolution property can only contain items of type \Travelport\Util\StructType\ExpertSolution, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\ExpertSolution[] $expertSolution
     * @return \Travelport\Util\StructType\ExpertSolutionList
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\ExpertSolution $item
     * @return \Travelport\Util\StructType\ExpertSolutionList
     */
    public function addToExpertSolution(\Travelport\Util\StructType\ExpertSolution $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\ExpertSolution) {
            throw new InvalidArgumentException(sprintf('The ExpertSolution property can only contain items of type \Travelport\Util\StructType\ExpertSolution, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
