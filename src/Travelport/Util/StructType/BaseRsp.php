...
class BaseRsp extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: ResponseMessage
     * @var \Travelport\Util\StructType\ResponseMessage[]
     */
     ...
     * @param \Travelport\Util\StructType\ResponseMessage[] $responseMessage
     ...
     * @return \Travelport\Util\StructType\ResponseMessage[]
     */
    ...
    public static function validateResponseMessageForArrayConstraintFromSetResponseMessage(?array $values = []): string
    {
        ...
        foreach ($values as $baseRspResponseMessageItem) {
            ...
            if (!$baseRspResponseMessageItem instanceof \Travelport\Util\StructType\ResponseMessage) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ResponseMessage property can only contain items of type \Travelport\Util\StructType\ResponseMessage, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\ResponseMessage[] $responseMessage
     * @return \Travelport\Util\StructType\BaseRsp
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\ResponseMessage $item
     * @return \Travelport\Util\StructType\BaseRsp
     */
    public function addToResponseMessage(\Travelport\Util\StructType\ResponseMessage $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\ResponseMessage) {
            throw new InvalidArgumentException(sprintf('The ResponseMessage property can only contain items of type \Travelport\Util\StructType\ResponseMessage, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
