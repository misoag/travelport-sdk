...
class AddressRestriction extends AbstractStructBase
{
    ...
    public static function validateRequiredFieldForArrayConstraintFromSetRequiredField(?array $values = []): string
    {
        ...
        foreach ($values as $addressRestrictionRequiredFieldItem) {
            ...
            if (!$addressRestrictionRequiredFieldItem instanceof \Travelport\Util\StructType\RequiredField) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The RequiredField property can only contain items of type \Travelport\Util\StructType\RequiredField, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \Travelport\Util\StructType\RequiredField[] $requiredField
     * @return \Travelport\Util\StructType\AddressRestriction
     */
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\RequiredField $item
     * @return \Travelport\Util\StructType\AddressRestriction
     */
    public function addToRequiredField(\Travelport\Util\StructType\RequiredField $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\RequiredField) {
            throw new InvalidArgumentException(sprintf('The RequiredField property can only contain items of type \Travelport\Util\StructType\RequiredField, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
