...
class MCOExchangeInfo extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: ExchangedCoupon
     * @var \Travelport\Util\StructType\ExchangedCoupon[]
     */
     ...
     * @param \Travelport\Util\StructType\ExchangedCoupon[] $exchangedCoupon
     * @param string $originalTicketNumber
     * @param string $originalCityCode
     * @param string $originalTicketDate
     * @param string $iATACode
     */
    public function __construct(?\Travelport\Util\StructType\FormOfPayment $formOfPayment = null, ?array $exchangedCoupon = null, ?string $originalTicketNumber = null, ?string $originalCityCode = null, ?string $originalTicketDate = null, ?string $iATACode = null)
    {
        $this
     ...
     * @param \Travelport\Util\StructType\FormOfPayment $formOfPayment
     * @return \Travelport\Util\StructType\MCOExchangeInfo
     */
    ...
    public static function validateExchangedCouponForArrayConstraintsFromSetExchangedCoupon(?array $values = []): string
    {
        ...
        foreach ($values as $mCOExchangeInfoExchangedCouponItem) {
            ...
            if (!$mCOExchangeInfoExchangedCouponItem instanceof \Travelport\Util\StructType\ExchangedCoupon) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ExchangedCoupon property can only contain items of type \Travelport\Util\StructType\ExchangedCoupon, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \Travelport\Util\StructType\ExchangedCoupon $item
     * @return \Travelport\Util\StructType\MCOExchangeInfo
     */
    public function addToExchangedCoupon(\Travelport\Util\StructType\ExchangedCoupon $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \Travelport\Util\StructType\ExchangedCoupon) {
            throw new InvalidArgumentException(sprintf('The ExchangedCoupon property can only contain items of type \Travelport\Util\StructType\ExchangedCoupon, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(4)
