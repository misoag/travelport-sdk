...
class CommissionRemark extends AbstractStructBase
{
     ...
     * - choiceMaxOccurs: 1
     * - choiceMinOccurs: 1
     * @var \StructType\ProviderReservationLevel|null
     */
    protected ?\StructType\ProviderReservationLevel $ProviderReservationLevel = null;
    /**
     ...
     * - choiceMinOccurs: 1
     * - maxOccurs: 4
     * @var \StructType\PassengerTypeLevel[]
     */
     ...
     * @param \StructType\ProviderReservationLevel $providerReservationLevel
     * @param \StructType\PassengerTypeLevel[] $passengerTypeLevel
     * @param string $key
     ...
     */
    public function __construct(?\StructType\ProviderReservationLevel $providerReservationLevel = null, ?array $passengerTypeLevel = null, ?string $key = null, ?string $providerReservationInfoRef = null, ?string $providerCode = null, ?string $elStat = null, ?bool $keyOverride = null)
    {
        $this
     ...
     * @return \StructType\ProviderReservationLevel|null
     */
    public function getProviderReservationLevel(): ?\StructType\ProviderReservationLevel
    {
     ...
     * @param \StructType\ProviderReservationLevel $providerReservationLevel
     * @return \StructType\CommissionRemark
     */
    public function setProviderReservationLevel(?\StructType\ProviderReservationLevel $providerReservationLevel = null): self
    {
     ...
     * @return \StructType\PassengerTypeLevel[]
     */
    ...
    public static function validatePassengerTypeLevelForArrayConstraintsFromSetPassengerTypeLevel(?array $values = []): string
    {
        ...
        foreach ($values as $commissionRemarkPassengerTypeLevelItem) {
            ...
            if (!$commissionRemarkPassengerTypeLevelItem instanceof \StructType\PassengerTypeLevel) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PassengerTypeLevel property can only contain items of type \StructType\PassengerTypeLevel, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\PassengerTypeLevel $item
     * @return \StructType\CommissionRemark
     */
    public function addToPassengerTypeLevel(\StructType\PassengerTypeLevel $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\PassengerTypeLevel) {
            throw new InvalidArgumentException(sprintf('The PassengerTypeLevel property can only contain items of type \StructType\PassengerTypeLevel, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: choice(ProviderReservationLevel, PassengerTypeLevel)
