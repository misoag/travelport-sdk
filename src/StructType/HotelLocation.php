...
class HotelLocation extends AbstractStructBase
{
     ...
     * Set Location value
     * @param string $location
     * @return \StructType\HotelLocation
     */
     ...
     * @uses \EnumType\TypeHotelLocation::valueIsValid()
     * @uses \EnumType\TypeHotelLocation::getValidValues()
    ...
    public function setLocationType(?string $locationType = null): self
    {
        ...
        if (!\EnumType\TypeHotelLocation::valueIsValid($locationType)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeHotelLocation', is_array($locationType) ? implode(', ', $locationType) : var_export($locationType, true), implode(', ', \EnumType\TypeHotelLocation::getValidValues())), __LINE__);
        }
