...
class Group extends AbstractStructBase
{
     ...
     * @param \StructType\SSRRef[] $sSRRef
     ...
     */
    public function __construct(int $groupSize, ?\StructType\Name $name = null, ?\StructType\DeliveryInfo $deliveryInfo = null, ?array $phoneNumber = null, ?array $sSRRef = null, ?\StructType\TypeStructuredAddress $address = null, ?array $bookingTravelerRef = null, ?string $key = null, ?string $travelerType = null, ?string $elStat = null, ?bool $keyOverride = null)
    {
        $this
    ...
    public static function validatePhoneNumberForArrayConstraintsFromSetPhoneNumber(?array $values = []): string
    {
        ...
        foreach ($values as $groupPhoneNumberItem) {
            ...
            if (!$groupPhoneNumberItem instanceof \StructType\PhoneNumber) {
    ...
    public static function validateSSRRefForArrayConstraintsFromSetSSRRef(?array $values = []): string
    {
        ...
        foreach ($values as $groupSSRRefItem) {
            // validation for constraint: itemType
            if (!$groupSSRRefItem instanceof \StructType\SSRRef) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SSRRef property can only contain items of type \StructType\SSRRef, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\SSRRef $item
     * @return \StructType\Group
     */
    public function addToSSRRef(\StructType\SSRRef $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\SSRRef) {
            throw new InvalidArgumentException(sprintf('The SSRRef property can only contain items of type \StructType\SSRRef, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateBookingTravelerRefForArrayConstraintsFromSetBookingTravelerRef(?array $values = []): string
    {
        ...
        foreach ($values as $groupBookingTravelerRefItem) {
            // validation for constraint: itemType
            if (!$groupBookingTravelerRefItem instanceof \StructType\BookingTravelerRef) {
