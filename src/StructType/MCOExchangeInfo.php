...
class MCOExchangeInfo extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: ExchangedCoupon
     * @var \StructType\ExchangedCoupon[]
     */
     ...
     * @param \StructType\ExchangedCoupon[] $exchangedCoupon
     * @param string $originalTicketNumber
     * @param string $originalCityCode
     * @param string $originalTicketDate
     * @param string $iATACode
     */
    public function __construct(?\StructType\FormOfPayment $formOfPayment = null, ?array $exchangedCoupon = null, ?string $originalTicketNumber = null, ?string $originalCityCode = null, ?string $originalTicketDate = null, ?string $iATACode = null)
    {
        $this
     ...
     * @param \StructType\FormOfPayment $formOfPayment
     * @return \StructType\MCOExchangeInfo
     */
     ...
     * @return \StructType\ExchangedCoupon[]
     */
    ...
    public static function validateExchangedCouponForArrayConstraintsFromSetExchangedCoupon(?array $values = []): string
    {
        ...
        foreach ($values as $mCOExchangeInfoExchangedCouponItem) {
            ...
            if (!$mCOExchangeInfoExchangedCouponItem instanceof \StructType\ExchangedCoupon) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ExchangedCoupon property can only contain items of type \StructType\ExchangedCoupon, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\ExchangedCoupon $item
     * @return \StructType\MCOExchangeInfo
     */
    public function addToExchangedCoupon(\StructType\ExchangedCoupon $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\ExchangedCoupon) {
            throw new InvalidArgumentException(sprintf('The ExchangedCoupon property can only contain items of type \StructType\ExchangedCoupon, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(4)
