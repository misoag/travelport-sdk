...
class MediaItem extends AbstractStructBase
{
    ...
    public static function validateSizeCodeForUnionConstraintsFromSetSizeCode($value): string
    {
        ...
        // validation for constraint: enumeration
        if (!\EnumType\TypeOtherImageSize::valueIsValid($value)) {
            $exception1 = new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeOtherImageSize', is_array($value) ? implode(', ', $value) : var_export($value, true), implode(', ', \EnumType\TypeOtherImageSize::getValidValues())), __LINE__);
        }
