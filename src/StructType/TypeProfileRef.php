...
class TypeProfileRef extends AbstractStructBase
{
     ...
     * @param string $profileEntityID
     * @return \StructType\TypeProfileRef
     */
     ...
     * @uses \EnumType\TypeProfileLevel::valueIsValid()
     * @uses \EnumType\TypeProfileLevel::getValidValues()
    ...
    public function setProfileLevel(string $profileLevel): self
    {
        ...
        if (!\EnumType\TypeProfileLevel::valueIsValid($profileLevel)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeProfileLevel', is_array($profileLevel) ? implode(', ', $profileLevel) : var_export($profileLevel, true), implode(', ', \EnumType\TypeProfileLevel::getValidValues())), __LINE__);
        }
