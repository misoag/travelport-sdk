...
class TypePassengerType extends AbstractStructBase
{
     ...
     * @var \StructType\PersonalGeography|null
     */
    protected ?\StructType\PersonalGeography $PersonalGeography = null;
    /**
     * The Age
     * Meta information extracted from the WSDL
     * - use: optional
     ...
     * @param \StructType\PersonalGeography $personalGeography
     * @param int $age
     * @param string $dOB
     * @param string $gender
     ...
     * Set Code value
     * @param string $code
     * @return \StructType\TypePassengerType
     */
    ...
    public static function validateLoyaltyCardForArrayConstraintsFromSetLoyaltyCard(?array $values = []): string
    {
        ...
        foreach ($values as $typePassengerTypeLoyaltyCardItem) {
            ...
            if (!$typePassengerTypeLoyaltyCardItem instanceof \StructType\LoyaltyCard) {
     ...
     * @return \StructType\DiscountCard[]
     */
    ...
    public static function validateDiscountCardForArrayConstraintsFromSetDiscountCard(?array $values = []): string
    {
        ...
        foreach ($values as $typePassengerTypeDiscountCardItem) {
            // validation for constraint: itemType
            if (!$typePassengerTypeDiscountCardItem instanceof \StructType\DiscountCard) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The DiscountCard property can only contain items of type \StructType\DiscountCard, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\DiscountCard $item
     * @return \StructType\TypePassengerType
     */
    public function addToDiscountCard(\StructType\DiscountCard $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\DiscountCard) {
            throw new InvalidArgumentException(sprintf('The DiscountCard property can only contain items of type \StructType\DiscountCard, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(9)
     ...
     * @return \StructType\PersonalGeography|null
     */
    public function getPersonalGeography(): ?\StructType\PersonalGeography
    {
     ...
     * @return \StructType\TypePassengerType
     */
    public function setPersonalGeography(?\StructType\PersonalGeography $personalGeography = null): self
    {
     ...
     * @uses \EnumType\TypeResidency::valueIsValid()
     * @uses \EnumType\TypeResidency::getValidValues()
     * @throws InvalidArgumentException
     * @param string $residencyType
     * @return \StructType\TypePassengerType
     */
    public function setResidencyType(?string $residencyType = null): self
    {
        ...
        if (!\EnumType\TypeResidency::valueIsValid($residencyType)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeResidency', is_array($residencyType) ? implode(', ', $residencyType) : var_export($residencyType, true), implode(', ', \EnumType\TypeResidency::getValidValues())), __LINE__);
        }
