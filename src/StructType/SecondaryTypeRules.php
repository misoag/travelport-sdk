...
class SecondaryTypeRules extends AbstractStructBase
{
     ...
     * Meta information extracted from the WSDL
     * - maxOccurs: 999
     * @var \StructType\SecondaryTypeRule[]
     */
     ...
     * @param \StructType\SecondaryTypeRule[] $secondaryTypeRule
     */
    ...
    public static function validateSecondaryTypeRuleForArrayConstraintsFromSetSecondaryTypeRule(?array $values = []): string
    {
        ...
        foreach ($values as $secondaryTypeRulesSecondaryTypeRuleItem) {
            ...
            if (!$secondaryTypeRulesSecondaryTypeRuleItem instanceof \StructType\SecondaryTypeRule) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SecondaryTypeRule property can only contain items of type \StructType\SecondaryTypeRule, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\SecondaryTypeRule[] $secondaryTypeRule
     * @return \StructType\SecondaryTypeRules
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\SecondaryTypeRule $item
     * @return \StructType\SecondaryTypeRules
     */
    public function addToSecondaryTypeRule(\StructType\SecondaryTypeRule $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\SecondaryTypeRule) {
            throw new InvalidArgumentException(sprintf('The SecondaryTypeRule property can only contain items of type \StructType\SecondaryTypeRule, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
