...
class HostTokenList extends AbstractStructBase
{
    ...
    public static function validateHostTokenForArrayConstraintsFromSetHostToken(?array $values = []): string
    {
        ...
        foreach ($values as $hostTokenListHostTokenItem) {
            ...
            if (!$hostTokenListHostTokenItem instanceof \StructType\HostToken) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The HostToken property can only contain items of type \StructType\HostToken, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\HostToken[] $hostToken
     * @return \StructType\HostTokenList
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\HostToken $item
     * @return \StructType\HostTokenList
     */
    public function addToHostToken(\StructType\HostToken $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\HostToken) {
            throw new InvalidArgumentException(sprintf('The HostToken property can only contain items of type \StructType\HostToken, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
