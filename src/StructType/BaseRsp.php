...
class BaseRsp extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: ResponseMessage
     * @var \StructType\ResponseMessage[]
     */
     ...
     * @param \StructType\ResponseMessage[] $responseMessage
    ...
    public static function validateResponseMessageForArrayConstraintsFromSetResponseMessage(?array $values = []): string
    {
        ...
        foreach ($values as $baseRspResponseMessageItem) {
            ...
            if (!$baseRspResponseMessageItem instanceof \StructType\ResponseMessage) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ResponseMessage property can only contain items of type \StructType\ResponseMessage, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\ResponseMessage $item
     * @return \StructType\BaseRsp
     */
    public function addToResponseMessage(\StructType\ResponseMessage $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\ResponseMessage) {
            throw new InvalidArgumentException(sprintf('The ResponseMessage property can only contain items of type \StructType\ResponseMessage, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
