...
class TravelComplianceData extends AbstractStructBase
{
     ...
     * - maxOccurs: 2
     * - minOccurs: 0
     * @var \StructType\PolicyCompliance[]
     */
     ...
     * - maxOccurs: 2
     * - minOccurs: 0
     * @var \StructType\ContractCompliance[]
     */
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \StructType\PreferredSupplier[]
     */
     ...
     * @param \StructType\PolicyCompliance[] $policyCompliance
     * @param \StructType\ContractCompliance[] $contractCompliance
     * @param \StructType\PreferredSupplier[] $preferredSupplier
     * @param string $key
     ...
     * @return \StructType\PolicyCompliance[]
     */
    ...
    public static function validatePolicyComplianceForArrayConstraintsFromSetPolicyCompliance(?array $values = []): string
    {
        ...
        foreach ($values as $travelComplianceDataPolicyComplianceItem) {
            ...
            if (!$travelComplianceDataPolicyComplianceItem instanceof \StructType\PolicyCompliance) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PolicyCompliance property can only contain items of type \StructType\PolicyCompliance, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\PolicyCompliance[] $policyCompliance
     * @return \StructType\TravelComplianceData
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\PolicyCompliance $item
     * @return \StructType\TravelComplianceData
     */
    public function addToPolicyCompliance(\StructType\PolicyCompliance $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\PolicyCompliance) {
            throw new InvalidArgumentException(sprintf('The PolicyCompliance property can only contain items of type \StructType\PolicyCompliance, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(2)
     ...
     * @return \StructType\ContractCompliance[]
     */
    ...
    public static function validateContractComplianceForArrayConstraintsFromSetContractCompliance(?array $values = []): string
    {
        ...
        foreach ($values as $travelComplianceDataContractComplianceItem) {
            // validation for constraint: itemType
            if (!$travelComplianceDataContractComplianceItem instanceof \StructType\ContractCompliance) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ContractCompliance property can only contain items of type \StructType\ContractCompliance, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\ContractCompliance $item
     * @return \StructType\TravelComplianceData
     */
    public function addToContractCompliance(\StructType\ContractCompliance $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\ContractCompliance) {
            throw new InvalidArgumentException(sprintf('The ContractCompliance property can only contain items of type \StructType\ContractCompliance, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(2)
    ...
    public static function validatePreferredSupplierForArrayConstraintsFromSetPreferredSupplier(?array $values = []): string
    {
        ...
        foreach ($values as $travelComplianceDataPreferredSupplierItem) {
            // validation for constraint: itemType
            if (!$travelComplianceDataPreferredSupplierItem instanceof \StructType\PreferredSupplier) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PreferredSupplier property can only contain items of type \StructType\PreferredSupplier, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\PreferredSupplier $item
     * @return \StructType\TravelComplianceData
     */
    public function addToPreferredSupplier(\StructType\PreferredSupplier $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\PreferredSupplier) {
            throw new InvalidArgumentException(sprintf('The PreferredSupplier property can only contain items of type \StructType\PreferredSupplier, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
