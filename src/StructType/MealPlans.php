...
class MealPlans extends AbstractStructBase
{
    ...
    public static function validateMealPlanForArrayConstraintsFromSetMealPlan(?array $values = []): string
    {
        ...
        foreach ($values as $mealPlansMealPlanItem) {
            ...
            if (!$mealPlansMealPlanItem instanceof \StructType\MealPlan) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MealPlan property can only contain items of type \StructType\MealPlan, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\MealPlan $item
     * @return \StructType\MealPlans
     */
    public function addToMealPlan(\StructType\MealPlan $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\MealPlan) {
            throw new InvalidArgumentException(sprintf('The MealPlan property can only contain items of type \StructType\MealPlan, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
    ...
    public function setBreakfast(?string $breakfast = null): self
    {
        ...
        if (!\EnumType\TypeTrinary::valueIsValid($breakfast)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeTrinary', is_array($breakfast) ? implode(', ', $breakfast) : var_export($breakfast, true), implode(', ', \EnumType\TypeTrinary::getValidValues())), __LINE__);
        }
    ...
    {
        // validation for constraint: enumeration
        if (!\EnumType\TypeTrinary::valueIsValid($lunch)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeTrinary', is_array($lunch) ? implode(', ', $lunch) : var_export($lunch, true), implode(', ', \EnumType\TypeTrinary::getValidValues())), __LINE__);
        }
        $this->Lunch = $lunch;
    ...
    {
        // validation for constraint: enumeration
        if (!\EnumType\TypeTrinary::valueIsValid($dinner)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeTrinary', is_array($dinner) ? implode(', ', $dinner) : var_export($dinner, true), implode(', ', \EnumType\TypeTrinary::getValidValues())), __LINE__);
        }
        $this->Dinner = $dinner;
