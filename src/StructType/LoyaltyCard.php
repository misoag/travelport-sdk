...
class LoyaltyCard extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \StructType\TypeProviderReservationSpecificInfo[]
     */
     ...
     * @param \StructType\TypeProviderReservationSpecificInfo[] $providerReservationSpecificInfo
     * @param string $status
    ...
    public static function validateProviderReservationSpecificInfoForArrayConstraintsFromSetProviderReservationSpecificInfo(?array $values = []): string
    {
        ...
        foreach ($values as $loyaltyCardProviderReservationSpecificInfoItem) {
            ...
            if (!$loyaltyCardProviderReservationSpecificInfoItem instanceof \StructType\TypeProviderReservationSpecificInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ProviderReservationSpecificInfo property can only contain items of type \StructType\TypeProviderReservationSpecificInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\TypeProviderReservationSpecificInfo $item
     * @return \StructType\LoyaltyCard
     */
    public function addToProviderReservationSpecificInfo(\StructType\TypeProviderReservationSpecificInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\TypeProviderReservationSpecificInfo) {
            throw new InvalidArgumentException(sprintf('The ProviderReservationSpecificInfo property can only contain items of type \StructType\TypeProviderReservationSpecificInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
     ...
     * @uses \EnumType\TypeProduct::valueIsValid()
     * @uses \EnumType\TypeProduct::getValidValues()
     * @throws InvalidArgumentException
     * @param string $supplierType
     * @return \StructType\LoyaltyCard
     */
    public function setSupplierType(?string $supplierType = null): self
    {
        ...
        if (!\EnumType\TypeProduct::valueIsValid($supplierType)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeProduct', is_array($supplierType) ? implode(', ', $supplierType) : var_export($supplierType, true), implode(', ', \EnumType\TypeProduct::getValidValues())), __LINE__);
        }
