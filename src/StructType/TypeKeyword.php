...
class TypeKeyword extends AbstractStructBase
{
     ...
     * @param string $name
     * @param \StructType\TypeSubKey[] $subKey
     * @param string[] $text
     * @param string $number
    ...
    public static function validateSubKeyForArrayConstraintsFromSetSubKey(?array $values = []): string
    {
        ...
        foreach ($values as $typeKeywordSubKeyItem) {
            ...
            if (!$typeKeywordSubKeyItem instanceof \StructType\TypeSubKey) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SubKey property can only contain items of type \StructType\TypeSubKey, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\TypeSubKey $item
     * @return \StructType\TypeKeyword
     */
    public function addToSubKey(\StructType\TypeSubKey $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\TypeSubKey) {
            throw new InvalidArgumentException(sprintf('The SubKey property can only contain items of type \StructType\TypeSubKey, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(99)
