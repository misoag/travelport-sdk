...
class ShopInformation extends AbstractStructBase
{
     ...
     * @param \StructType\FlightsOffered[] $flightsOffered
    ...
    public static function validateSearchRequestForArrayConstraintsFromSetSearchRequest(?array $values = []): string
    {
        ...
        foreach ($values as $shopInformationSearchRequestItem) {
            ...
            if (!$shopInformationSearchRequestItem instanceof \StructType\SearchRequest) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SearchRequest property can only contain items of type \StructType\SearchRequest, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\SearchRequest[] $searchRequest
     * @return \StructType\ShopInformation
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\SearchRequest $item
     * @return \StructType\ShopInformation
     */
    public function addToSearchRequest(\StructType\SearchRequest $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\SearchRequest) {
            throw new InvalidArgumentException(sprintf('The SearchRequest property can only contain items of type \StructType\SearchRequest, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
     ...
     * @return \StructType\FlightsOffered[]
     */
    ...
    public static function validateFlightsOfferedForArrayConstraintsFromSetFlightsOffered(?array $values = []): string
    {
        ...
        foreach ($values as $shopInformationFlightsOfferedItem) {
            // validation for constraint: itemType
            if (!$shopInformationFlightsOfferedItem instanceof \StructType\FlightsOffered) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The FlightsOffered property can only contain items of type \StructType\FlightsOffered, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\FlightsOffered $item
     * @return \StructType\ShopInformation
     */
    public function addToFlightsOffered(\StructType\FlightsOffered $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\FlightsOffered) {
            throw new InvalidArgumentException(sprintf('The FlightsOffered property can only contain items of type \StructType\FlightsOffered, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
