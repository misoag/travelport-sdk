...
class ProhibitedChains extends AbstractStructBase
{
    ...
    public static function validateHotelChainForArrayConstraintsFromSetHotelChain(?array $values = []): string
    {
        ...
        foreach ($values as $prohibitedChainsHotelChainItem) {
            ...
            if (!$prohibitedChainsHotelChainItem instanceof \StructType\HotelChain) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The HotelChain property can only contain items of type \StructType\HotelChain, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\HotelChain[] $hotelChain
     * @return \StructType\ProhibitedChains
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\HotelChain $item
     * @return \StructType\ProhibitedChains
     */
    public function addToHotelChain(\StructType\HotelChain $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\HotelChain) {
            throw new InvalidArgumentException(sprintf('The HotelChain property can only contain items of type \StructType\HotelChain, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
