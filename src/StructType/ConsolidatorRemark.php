...
class ConsolidatorRemark extends AbstractStructBase
{
     ...
     * @param \StructType\PseudoCityCode[] $pseudoCityCode
     * @param string $key
     ...
     * @return \StructType\PseudoCityCode[]
     */
    ...
    public static function validatePseudoCityCodeForArrayConstraintsFromSetPseudoCityCode(?array $values = []): string
    {
        ...
        foreach ($values as $consolidatorRemarkPseudoCityCodeItem) {
            ...
            if (!$consolidatorRemarkPseudoCityCodeItem instanceof \StructType\PseudoCityCode) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PseudoCityCode property can only contain items of type \StructType\PseudoCityCode, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\PseudoCityCode[] $pseudoCityCode
     * @return \StructType\ConsolidatorRemark
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\PseudoCityCode $item
     * @return \StructType\ConsolidatorRemark
     */
    public function addToPseudoCityCode(\StructType\PseudoCityCode $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\PseudoCityCode) {
            throw new InvalidArgumentException(sprintf('The PseudoCityCode property can only contain items of type \StructType\PseudoCityCode, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(5)
