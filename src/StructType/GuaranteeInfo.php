...
class GuaranteeInfo extends AbstractStructBase
{
     ...
     * - minOccurs: 0
     * - ref: DepositAmount
     * @var \StructType\DepositAmount|null
     */
    protected ?\StructType\DepositAmount $DepositAmount = null;
    /**
     * The DepositNights
     * Meta information extracted from the WSDL
     ...
     * @var \StructType\GuaranteePaymentType[]
     */
     ...
     * @param \StructType\GuaranteePaymentType[] $guaranteePaymentType
     ...
     */
    public function __construct(?\StructType\DepositAmount $depositAmount = null, ?int $depositNights = null, ?int $depositPercent = null, ?array $guaranteePaymentType = null, ?string $absoluteDeadline = null, ?bool $credentialsRequired = null, ?string $holdTime = null, ?string $guaranteeType = null, ?string $offsetTimeUnit = null, ?int $offsetUnitMultiplier = null, ?string $offsetDropTime = null)
    {
        $this
     ...
     */
    public function getDepositAmount(): ?\StructType\DepositAmount
    {
     ...
     * @param \StructType\DepositAmount $depositAmount
     * @return \StructType\GuaranteeInfo
     */
    public function setDepositAmount(?\StructType\DepositAmount $depositAmount = null): self
    {
    ...
    public static function validateGuaranteePaymentTypeForArrayConstraintsFromSetGuaranteePaymentType(?array $values = []): string
    {
        ...
        foreach ($values as $guaranteeInfoGuaranteePaymentTypeItem) {
            ...
            if (!$guaranteeInfoGuaranteePaymentTypeItem instanceof \StructType\GuaranteePaymentType) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The GuaranteePaymentType property can only contain items of type \StructType\GuaranteePaymentType, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\GuaranteePaymentType $item
     * @return \StructType\GuaranteeInfo
     */
    public function addToGuaranteePaymentType(\StructType\GuaranteePaymentType $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\GuaranteePaymentType) {
            throw new InvalidArgumentException(sprintf('The GuaranteePaymentType property can only contain items of type \StructType\GuaranteePaymentType, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
