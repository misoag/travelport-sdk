...
class HotelKeywordReq extends BaseReq
{
     ...
     * @param \StructType\Keyword[] $keyword
     * @param \StructType\PermittedProviders $permittedProviders
     * @param string $hotelCode
     * @param string $checkinDate
     * @param bool $returnKeywordList
     */
    public function __construct(string $hotelChain, ?array $keyword = null, ?\StructType\PermittedProviders $permittedProviders = null, ?string $hotelCode = null, ?string $checkinDate = null, ?bool $returnKeywordList = null)
    {
        $this
     ...
     * @param string $hotelChain
     * @return \StructType\HotelKeywordReq
     */
    ...
    public static function validateKeywordForArrayConstraintsFromSetKeyword(?array $values = []): string
    {
        ...
        foreach ($values as $hotelKeywordReqKeywordItem) {
            ...
            if (!$hotelKeywordReqKeywordItem instanceof \StructType\Keyword) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The Keyword property can only contain items of type \StructType\Keyword, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\Keyword $item
     * @return \StructType\HotelKeywordReq
     */
    public function addToKeyword(\StructType\Keyword $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\Keyword) {
            throw new InvalidArgumentException(sprintf('The Keyword property can only contain items of type \StructType\Keyword, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(15)
