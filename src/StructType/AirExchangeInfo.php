...
class AirExchangeInfo extends AbstractStructBase
{
     ...
     * - maxOccurs: 1
     * - minOccurs: 0
     * @var \StructType\TotalPenaltyTaxInfo|null
     */
    protected ?\StructType\TotalPenaltyTaxInfo $TotalPenaltyTaxInfo = null;
    /**
     * The PaidTax
     * Meta information extracted from the WSDL
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \StructType\TypeTax[]
     */
     ...
     * @param \StructType\TotalPenaltyTaxInfo $totalPenaltyTaxInfo
     * @param \StructType\TypeTax[] $paidTax
     * @param \StructType\TicketFeeInfo[] $ticketFeeInfo
     * @param string[] $reason
     * @param \StructType\TypeFeeInfo[] $feeInfo
     * @param \StructType\TypeTaxInfo[] $taxInfo
     ...
     * @param string $exchangeAmount
     * @return \StructType\AirExchangeInfo
     */
     ...
     * @return \StructType\TotalPenaltyTaxInfo|null
     */
    public function getTotalPenaltyTaxInfo(): ?\StructType\TotalPenaltyTaxInfo
    {
     ...
     * @return \StructType\AirExchangeInfo
     */
    public function setTotalPenaltyTaxInfo(?\StructType\TotalPenaltyTaxInfo $totalPenaltyTaxInfo = null): self
    {
    ...
    public static function validatePaidTaxForArrayConstraintsFromSetPaidTax(?array $values = []): string
    {
        ...
        foreach ($values as $airExchangeInfoPaidTaxItem) {
            ...
            if (!$airExchangeInfoPaidTaxItem instanceof \StructType\TypeTax) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PaidTax property can only contain items of type \StructType\TypeTax, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\TypeTax $item
     * @return \StructType\AirExchangeInfo
     */
    public function addToPaidTax(\StructType\TypeTax $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\TypeTax) {
            throw new InvalidArgumentException(sprintf('The PaidTax property can only contain items of type \StructType\TypeTax, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
     ...
     * @return \StructType\TicketFeeInfo[]
     */
    ...
    public static function validateTicketFeeInfoForArrayConstraintsFromSetTicketFeeInfo(?array $values = []): string
    {
        ...
        foreach ($values as $airExchangeInfoTicketFeeInfoItem) {
            // validation for constraint: itemType
            if (!$airExchangeInfoTicketFeeInfoItem instanceof \StructType\TicketFeeInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The TicketFeeInfo property can only contain items of type \StructType\TicketFeeInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\TicketFeeInfo $item
     * @return \StructType\AirExchangeInfo
     */
    public function addToTicketFeeInfo(\StructType\TicketFeeInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\TicketFeeInfo) {
            throw new InvalidArgumentException(sprintf('The TicketFeeInfo property can only contain items of type \StructType\TicketFeeInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateFeeInfoForArrayConstraintsFromSetFeeInfo(?array $values = []): string
    {
        ...
        foreach ($values as $airExchangeInfoFeeInfoItem) {
            // validation for constraint: itemType
            if (!$airExchangeInfoFeeInfoItem instanceof \StructType\TypeFeeInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The FeeInfo property can only contain items of type \StructType\TypeFeeInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\TypeFeeInfo $item
     * @return \StructType\AirExchangeInfo
     */
    public function addToFeeInfo(\StructType\TypeFeeInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\TypeFeeInfo) {
            throw new InvalidArgumentException(sprintf('The FeeInfo property can only contain items of type \StructType\TypeFeeInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateTaxInfoForArrayConstraintsFromSetTaxInfo(?array $values = []): string
    {
        ...
        foreach ($values as $airExchangeInfoTaxInfoItem) {
            // validation for constraint: itemType
            if (!$airExchangeInfoTaxInfoItem instanceof \StructType\TypeTaxInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The TaxInfo property can only contain items of type \StructType\TypeTaxInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\TypeTaxInfo $item
     * @return \StructType\AirExchangeInfo
     */
    public function addToTaxInfo(\StructType\TypeTaxInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\TypeTaxInfo) {
            throw new InvalidArgumentException(sprintf('The TaxInfo property can only contain items of type \StructType\TypeTaxInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    /**
     * Set FarePull value
     * @uses \EnumType\TypeFarePull::valueIsValid()
     * @uses \EnumType\TypeFarePull::getValidValues()
     * @throws InvalidArgumentException
     * @param string $farePull
     * @return \StructType\AirExchangeInfo
     */
    public function setFarePull(?string $farePull = null): self
    {
        ...
        if (!\EnumType\TypeFarePull::valueIsValid($farePull)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeFarePull', is_array($farePull) ? implode(', ', $farePull) : var_export($farePull, true), implode(', ', \EnumType\TypeFarePull::getValidValues())), __LINE__);
        }
     ...
     * @uses \EnumType\TypeFormOfRefund::valueIsValid()
     * @uses \EnumType\TypeFormOfRefund::getValidValues()
     * @throws InvalidArgumentException
     * @param string $formOfRefund
     * @return \StructType\AirExchangeInfo
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\EnumType\TypeFormOfRefund::valueIsValid($formOfRefund)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeFormOfRefund', is_array($formOfRefund) ? implode(', ', $formOfRefund) : var_export($formOfRefund, true), implode(', ', \EnumType\TypeFormOfRefund::getValidValues())), __LINE__);
        }
