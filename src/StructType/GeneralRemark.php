...
class GeneralRemark extends AbstractStructBase
{
    ...
    /**
     * Set Direction value
     * @uses \EnumType\TypeDirection::valueIsValid()
     * @uses \EnumType\TypeDirection::getValidValues()
     * @throws InvalidArgumentException
     * @param string $direction
     * @return \StructType\GeneralRemark
     */
    ...
    {
        // validation for constraint: enumeration
        if (!\EnumType\TypeDirection::valueIsValid($direction)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeDirection', is_array($direction) ? implode(', ', $direction) : var_export($direction, true), implode(', ', \EnumType\TypeDirection::getValidValues())), __LINE__);
        }
