...
class TypeAgencyHierarchyReference extends AbstractStructBase
{
     ...
     * Set ProfileID value
     * @param int $profileID
     * @return \StructType\TypeAgencyHierarchyReference
     */
     ...
     * @uses \EnumType\TypeAgencyProfileLevel::valueIsValid()
     * @uses \EnumType\TypeAgencyProfileLevel::getValidValues()
    ...
    public function setProfileType(string $profileType): self
    {
        ...
        if (!\EnumType\TypeAgencyProfileLevel::valueIsValid($profileType)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeAgencyProfileLevel', is_array($profileType) ? implode(', ', $profileType) : var_export($profileType, true), implode(', ', \EnumType\TypeAgencyProfileLevel::getValidValues())), __LINE__);
        }
