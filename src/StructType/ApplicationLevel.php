...
class ApplicationLevel extends AbstractStructBase
{
     ...
     * Meta information extracted from the WSDL
     * - minOccurs: 0
     * @var \StructType\ApplicationLimits|null
     */
    protected ?\StructType\ApplicationLimits $ApplicationLimits = null;
    /**
     * The ServiceData
     * Meta information extracted from the WSDL
     * - maxOccurs: 999
     ...
     * @param \StructType\ApplicationLimits $applicationLimits
     * @param \StructType\ServiceData[] $serviceData
     * @param string[] $applicableLevels
     * @param string $providerDefinedApplicableLevels
     */
    public function __construct(?\StructType\ApplicationLimits $applicationLimits = null, ?array $serviceData = null, ?array $applicableLevels = null, ?string $providerDefinedApplicableLevels = null)
    {
        $this
     ...
     * @return \StructType\ApplicationLimits|null
     */
    public function getApplicationLimits(): ?\StructType\ApplicationLimits
    {
     ...
     * @param \StructType\ApplicationLimits $applicationLimits
     * @return \StructType\ApplicationLevel
     */
    public function setApplicationLimits(?\StructType\ApplicationLimits $applicationLimits = null): self
    {
    ...
    public static function validateServiceDataForArrayConstraintsFromSetServiceData(?array $values = []): string
    {
        ...
        foreach ($values as $applicationLevelServiceDataItem) {
            ...
            if (!$applicationLevelServiceDataItem instanceof \StructType\ServiceData) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ServiceData property can only contain items of type \StructType\ServiceData, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\ServiceData $item
     * @return \StructType\ApplicationLevel
     */
    public function addToServiceData(\StructType\ServiceData $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\ServiceData) {
            throw new InvalidArgumentException(sprintf('The ServiceData property can only contain items of type \StructType\ServiceData, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateApplicableLevelsForArrayConstraintsFromSetApplicableLevels(?array $values = []): string
    {
        ...
        foreach ($values as $applicationLevelApplicableLevelsItem) {
            ...
            if (!\EnumType\OptionalServiceApplicabilityType::valueIsValid($applicationLevelApplicableLevelsItem)) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\OptionalServiceApplicabilityType', is_array($invalidValues) ? implode(', ', $invalidValues) : var_export($invalidValues, true), implode(', ', \EnumType\OptionalServiceApplicabilityType::getValidValues()));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: enumeration
        if (!\EnumType\OptionalServiceApplicabilityType::valueIsValid($item)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\OptionalServiceApplicabilityType', is_array($item) ? implode(', ', $item) : var_export($item, true), implode(', ', \EnumType\OptionalServiceApplicabilityType::getValidValues())), __LINE__);
        }
