...
class RequestKeyMappings extends AbstractStructBase
{
    ...
    public static function validateKeyMappingForArrayConstraintsFromSetKeyMapping(?array $values = []): string
    {
        ...
        foreach ($values as $requestKeyMappingsKeyMappingItem) {
            ...
            if (!$requestKeyMappingsKeyMappingItem instanceof \StructType\KeyMapping) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The KeyMapping property can only contain items of type \StructType\KeyMapping, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\KeyMapping[] $keyMapping
     * @return \StructType\RequestKeyMappings
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\KeyMapping $item
     * @return \StructType\RequestKeyMappings
     */
    public function addToKeyMapping(\StructType\KeyMapping $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\KeyMapping) {
            throw new InvalidArgumentException(sprintf('The KeyMapping property can only contain items of type \StructType\KeyMapping, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
