...
class CancelInfo extends AbstractStructBase
{
    ...
    public function setNonRefundableStayIndicator(?string $nonRefundableStayIndicator = null): self
    {
        ...
        if (!\EnumType\TypeTrinary::valueIsValid($nonRefundableStayIndicator)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeTrinary', is_array($nonRefundableStayIndicator) ? implode(', ', $nonRefundableStayIndicator) : var_export($nonRefundableStayIndicator, true), implode(', ', \EnumType\TypeTrinary::getValidValues())), __LINE__);
        }
