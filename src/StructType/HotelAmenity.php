...
class HotelAmenity extends AbstractStructBase
{
    ...
    /**
     * Set Level value
     * @uses \EnumType\TypeAmenityLevel::valueIsValid()
     * @uses \EnumType\TypeAmenityLevel::getValidValues()
    ...
    public function setLevel(string $level): self
    {
        ...
        if (!\EnumType\TypeAmenityLevel::valueIsValid($level)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\TypeAmenityLevel', is_array($level) ? implode(', ', $level) : var_export($level, true), implode(', ', \EnumType\TypeAmenityLevel::getValidValues())), __LINE__);
        }
        $this->Level = $level;
