...
class ModifyRules extends AbstractStructBase
{
     ...
     * @param \StructType\ModifyRule[] $modifyRule
    ...
    public static function validateModifyRuleForArrayConstraintsFromSetModifyRule(?array $values = []): string
    {
        ...
        foreach ($values as $modifyRulesModifyRuleItem) {
            ...
            if (!$modifyRulesModifyRuleItem instanceof \StructType\ModifyRule) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The ModifyRule property can only contain items of type \StructType\ModifyRule, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\ModifyRule $item
     * @return \StructType\ModifyRules
     */
    public function addToModifyRule(\StructType\ModifyRule $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\ModifyRule) {
            throw new InvalidArgumentException(sprintf('The ModifyRule property can only contain items of type \StructType\ModifyRule, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateSupportedModificationsForArrayConstraintsFromSetSupportedModifications(?array $values = []): string
    {
        ...
        foreach ($values as $modifyRulesSupportedModificationsItem) {
            ...
            if (!\EnumType\ModificationType::valueIsValid($modifyRulesSupportedModificationsItem)) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\ModificationType', is_array($invalidValues) ? implode(', ', $invalidValues) : var_export($invalidValues, true), implode(', ', \EnumType\ModificationType::getValidValues()));
        }
        unset($invalidValues);
    ...
    {
        // validation for constraint: enumeration
        if (!\EnumType\ModificationType::valueIsValid($item)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \EnumType\ModificationType', is_array($item) ? implode(', ', $item) : var_export($item, true), implode(', ', \EnumType\ModificationType::getValidValues())), __LINE__);
        }
