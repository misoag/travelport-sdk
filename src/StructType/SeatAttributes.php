...
class SeatAttributes extends AbstractStructBase
{
     ...
     * @return \StructType\SeatAttribute[]
     */
    ...
    public static function validateSeatAttributeForArrayConstraintsFromSetSeatAttribute(?array $values = []): string
    {
        ...
        foreach ($values as $seatAttributesSeatAttributeItem) {
            ...
            if (!$seatAttributesSeatAttributeItem instanceof \StructType\SeatAttribute) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The SeatAttribute property can only contain items of type \StructType\SeatAttribute, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\SeatAttribute[] $seatAttribute
     * @return \StructType\SeatAttributes
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\SeatAttribute $item
     * @return \StructType\SeatAttributes
     */
    public function addToSeatAttribute(\StructType\SeatAttribute $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\SeatAttribute) {
            throw new InvalidArgumentException(sprintf('The SeatAttribute property can only contain items of type \StructType\SeatAttribute, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(10)
