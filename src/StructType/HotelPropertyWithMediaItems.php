...
class HotelPropertyWithMediaItems extends AbstractStructBase
{
     ...
     * @param \StructType\TypeResultMessage[] $mediaResultMessage
     */
    public function __construct(?\StructType\HotelProperty $hotelProperty = null, ?array $mediaItem = null, ?array $mediaResultMessage = null)
    {
        $this
    ...
    public static function validateMediaItemForArrayConstraintsFromSetMediaItem(?array $values = []): string
    {
        ...
        foreach ($values as $hotelPropertyWithMediaItemsMediaItemItem) {
            ...
            if (!$hotelPropertyWithMediaItemsMediaItemItem instanceof \StructType\MediaItem) {
    ...
    public static function validateMediaResultMessageForArrayConstraintsFromSetMediaResultMessage(?array $values = []): string
    {
        ...
        foreach ($values as $hotelPropertyWithMediaItemsMediaResultMessageItem) {
            // validation for constraint: itemType
            if (!$hotelPropertyWithMediaItemsMediaResultMessageItem instanceof \StructType\TypeResultMessage) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The MediaResultMessage property can only contain items of type \StructType\TypeResultMessage, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @return \StructType\HotelPropertyWithMediaItems
     */
    public function addToMediaResultMessage(\StructType\TypeResultMessage $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\TypeResultMessage) {
            throw new InvalidArgumentException(sprintf('The MediaResultMessage property can only contain items of type \StructType\TypeResultMessage, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
