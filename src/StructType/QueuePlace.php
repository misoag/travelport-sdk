...
class QueuePlace extends AbstractStructBase
{
     ...
     * @param \StructType\QueueSelector[] $queueSelector
     */
    ...
    public static function validateQueueSelectorForArrayConstraintsFromSetQueueSelector(?array $values = []): string
    {
        ...
        foreach ($values as $queuePlaceQueueSelectorItem) {
            ...
            if (!$queuePlaceQueueSelectorItem instanceof \StructType\QueueSelector) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The QueueSelector property can only contain items of type \StructType\QueueSelector, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\QueueSelector $item
     * @return \StructType\QueuePlace
     */
    public function addToQueueSelector(\StructType\QueueSelector $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\QueueSelector) {
            throw new InvalidArgumentException(sprintf('The QueueSelector property can only contain items of type \StructType\QueueSelector, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
