...
class TotalPenaltyTaxInfo extends AbstractStructBase
{
     ...
     * @param \StructType\TypeTax[] $penaltyTaxInfo
    ...
    public static function validatePenaltyTaxInfoForArrayConstraintsFromSetPenaltyTaxInfo(?array $values = []): string
    {
        ...
        foreach ($values as $totalPenaltyTaxInfoPenaltyTaxInfoItem) {
            ...
            if (!$totalPenaltyTaxInfoPenaltyTaxInfoItem instanceof \StructType\TypeTax) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The PenaltyTaxInfo property can only contain items of type \StructType\TypeTax, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\TypeTax[] $penaltyTaxInfo
     * @return \StructType\TotalPenaltyTaxInfo
     */
     ...
     * @return \StructType\TotalPenaltyTaxInfo
     */
    public function addToPenaltyTaxInfo(\StructType\TypeTax $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\TypeTax) {
            throw new InvalidArgumentException(sprintf('The PenaltyTaxInfo property can only contain items of type \StructType\TypeTax, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
