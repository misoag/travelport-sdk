...
class PaymentRestriction extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - ref: CardRestriction
     * @var \StructType\CardRestriction[]
     */
     ...
     * @var \StructType\AddressRestriction|null
     */
    protected ?\StructType\AddressRestriction $AddressRestriction = null;
    /**
     ...
     * @param \StructType\CardRestriction[] $cardRestriction
     * @param \StructType\AddressRestriction $addressRestriction
     */
    public function __construct(?array $cardRestriction = null, ?\StructType\AddressRestriction $addressRestriction = null)
    {
        $this
    ...
    public static function validateCardRestrictionForArrayConstraintsFromSetCardRestriction(?array $values = []): string
    {
        ...
        foreach ($values as $paymentRestrictionCardRestrictionItem) {
            ...
            if (!$paymentRestrictionCardRestrictionItem instanceof \StructType\CardRestriction) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The CardRestriction property can only contain items of type \StructType\CardRestriction, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @param \StructType\CardRestriction[] $cardRestriction
     * @return \StructType\PaymentRestriction
     */
     ...
     * @throws InvalidArgumentException
     * @param \StructType\CardRestriction $item
     * @return \StructType\PaymentRestriction
     */
    public function addToCardRestriction(\StructType\CardRestriction $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\CardRestriction) {
            throw new InvalidArgumentException(sprintf('The CardRestriction property can only contain items of type \StructType\CardRestriction, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
     ...
     * @return \StructType\AddressRestriction|null
     */
    public function getAddressRestriction(): ?\StructType\AddressRestriction
    {
     ...
     * @return \StructType\PaymentRestriction
     */
    public function setAddressRestriction(?\StructType\AddressRestriction $addressRestriction = null): self
    {
