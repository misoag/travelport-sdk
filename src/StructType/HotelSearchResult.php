...
class HotelSearchResult extends AbstractStructBase
{
     ...
     * - maxOccurs: 999
     * - minOccurs: 0
     * @var \StructType\HotelSearchError[]
     */
     ...
     * @var \StructType\MediaItem|null
     */
    protected ?\StructType\MediaItem $MediaItem = null;
    /**
     * The HotelType
     * Meta information extracted from the WSDL
     * - minOccurs: 0
     ...
     * @var \StructType\PropertyDescription|null
     */
    protected ?\StructType\PropertyDescription $PropertyDescription = null;
    /**
     ...
     * @param \StructType\HotelSearchError[] $hotelSearchError
     * @param \StructType\CorporateDiscountID[] $corporateDiscountID
     * @param \StructType\RateInfo[] $rateInfo
     * @param \StructType\MediaItem $mediaItem
     * @param \StructType\HotelType $hotelType
     * @param \StructType\PropertyDescription $propertyDescription
     */
    public function __construct(array $hotelProperty, ?array $vendorLocation = null, ?array $hotelSearchError = null, ?array $corporateDiscountID = null, ?array $rateInfo = null, ?\StructType\MediaItem $mediaItem = null, ?\StructType\HotelType $hotelType = null, ?\StructType\PropertyDescription $propertyDescription = null)
    {
        $this
    ...
    public static function validateHotelPropertyForArrayConstraintsFromSetHotelProperty(?array $values = []): string
    {
        ...
        foreach ($values as $hotelSearchResultHotelPropertyItem) {
            ...
            if (!$hotelSearchResultHotelPropertyItem instanceof \StructType\HotelProperty) {
    ...
    public static function validateVendorLocationForArrayConstraintsFromSetVendorLocation(?array $values = []): string
    {
        ...
        foreach ($values as $hotelSearchResultVendorLocationItem) {
            // validation for constraint: itemType
            if (!$hotelSearchResultVendorLocationItem instanceof \StructType\VendorLocation) {
    ...
    public static function validateHotelSearchErrorForArrayConstraintsFromSetHotelSearchError(?array $values = []): string
    {
        ...
        foreach ($values as $hotelSearchResultHotelSearchErrorItem) {
            // validation for constraint: itemType
            if (!$hotelSearchResultHotelSearchErrorItem instanceof \StructType\HotelSearchError) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The HotelSearchError property can only contain items of type \StructType\HotelSearchError, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \StructType\HotelSearchError $item
     * @return \StructType\HotelSearchResult
     */
    public function addToHotelSearchError(\StructType\HotelSearchError $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\HotelSearchError) {
            throw new InvalidArgumentException(sprintf('The HotelSearchError property can only contain items of type \StructType\HotelSearchError, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    public static function validateCorporateDiscountIDForArrayConstraintsFromSetCorporateDiscountID(?array $values = []): string
    {
        ...
        foreach ($values as $hotelSearchResultCorporateDiscountIDItem) {
            // validation for constraint: itemType
            if (!$hotelSearchResultCorporateDiscountIDItem instanceof \StructType\CorporateDiscountID) {
    ...
    public static function validateRateInfoForArrayConstraintsFromSetRateInfo(?array $values = []): string
    {
        ...
        foreach ($values as $hotelSearchResultRateInfoItem) {
            // validation for constraint: itemType
            if (!$hotelSearchResultRateInfoItem instanceof \StructType\RateInfo) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The RateInfo property can only contain items of type \StructType\RateInfo, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @return \StructType\HotelSearchResult
     */
    public function addToRateInfo(\StructType\RateInfo $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \StructType\RateInfo) {
            throw new InvalidArgumentException(sprintf('The RateInfo property can only contain items of type \StructType\RateInfo, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
        // validation for constraint: maxOccurs(999)
    ...
    /**
     * Get MediaItem value
     * @return \StructType\MediaItem|null
     */
    public function getMediaItem(): ?\StructType\MediaItem
    {
     ...
     * @return \StructType\HotelSearchResult
     */
    public function setMediaItem(?\StructType\MediaItem $mediaItem = null): self
    {
     ...
     * @return \StructType\PropertyDescription|null
     */
    public function getPropertyDescription(): ?\StructType\PropertyDescription
    {
     ...
     * @return \StructType\HotelSearchResult
     */
    public function setPropertyDescription(?\StructType\PropertyDescription $propertyDescription = null): self
    {
